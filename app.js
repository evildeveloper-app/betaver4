...
function _authFails(){ try { return Number(CacheService.getScriptCache().get('auth_fails') || 0); } catch(e){ return 0; } }
function _authFailBump(){ try { var c = CacheService.getScriptCache(); c.put('auth_fails', String(_authFails()+1), 600); } catch(e){} }
function _checkAuth(k, action){
  var s = _secretVal(); if(!s) return true; if(action==='setSecret') return true;
  if (_authFails() >= 10) return false;
  if (String(k||'') === s) return true;
  _authFailBump();
  return false;
}
...
var ACTIONS = {
  ...
  setSecret: function(b) {
    ...
    if (cur && _authFails() >= 10) return {error:'locked · too many wrong guesses, try later'};
    if (cur && String(b.current||'') !== cur) { _authFailBump(); return {error:'wrong password'}; }
...
// ── Apple-style Settings: each category row drills into its own detail sub-page ──
function closeAllSettingsDetails(){
  document.querySelectorAll('.settings-detail.open').forEach(el => el.classList.remove('open'));
}
function openSettingsCat(id){
  const p = document.getElementById('sd-' + id);
  if (!p) return;
  try { renderSettings(); } catch(e) {}   // populate the sub-page's labels/toggles first
  closeAllSettingsDetails();
  p.classList.add('open');
  try { p.scrollTop = 0; } catch(e) {}
}
function closeSettingsCat(){
  closeAllSettingsDetails();
  try { renderSettings(); } catch(e) {}    // reflect any changes back onto the menu
}
...
function renderPersonalize() {
  ...
  const savedTheme = localStorage.getItem('prizm_theme') || 'light';
...
// Side-rail date tab (Ducat-style): "Feb 03" running vertically down the row's left edge.
function fmtDateRail(s){
  const d = parseDate(s);
...
function reapplyPrefsToUI(){
  ...
  try { applyTheme(localStorage.getItem('prizm_theme') || 'light'); } catch(e) {}
...
function _maybeBackupReminder() {
  try {
    if (S.mode !== 'local') return;
    if ((S.txs || []).length < 10) return;
    const last = Number(localStorage.getItem('prizm_last_backup') || 0);
    const nag  = Number(localStorage.getItem('prizm_backup_nag') || 0);
    const TWO_WEEKS = 14 * 864e5;
    if (Date.now() - last < TWO_WEEKS) return;
    if (Date.now() - nag  < TWO_WEEKS) return;
    localStorage.setItem('prizm_backup_nag', String(Date.now()));
    setTimeout(() => { try { toast('💾 Your data lives only on this device \u2014 back it up in Settings \u203a Data & Sync'); } catch (e) {} }, 2500);
  } catch (e) {}
}
...
function boot() {
  ...
  try { maybeRevalidateLicense(); } catch(e) {} // quiet subscription re-check
  ...
  applyTheme(localStorage.getItem('prizm_theme') || 'light'); // light (white + gold) is the default
  ...
  if (S.mode === 'local') { enterApp(); _maybeBackupReminder(); return; }
  if (S.mode === 'firebase') {
...
function enterApp() {
  ...
  renderAll();
  playEntrance();          // staggered fade-in so the app feels freshly built for them
  firstRunWelcome();       // one-time personalised greeting
  healRecurring();
  checkRecurringDue();
  weeklyInsightNudge();
  try { setTimeout(handleQuickAddURL, 400); } catch(e) {}
  try { setTimeout(maybeShowWrapped, 1400); } catch(e) {}
}

// ── Entrance motion ──────────────────────────────────────────────────────────
// Adds a stagger class to a page so its direct children rise+fade in sequence.
// Self-cleaning, so it can replay on every tab visit without leaving state behind.
function playEntrance(id) {
  try {
    const p = document.getElementById(id || 'page-home');
    if (!p) return;
    p.classList.remove('pz-enter');
    void p.offsetWidth;               // force reflow so the animation restarts
    p.classList.add('pz-enter');
    setTimeout(() => { try { p.classList.remove('pz-enter'); } catch(e) {} }, 1400);
  } catch(e) {}
}
// First launch after setup: greet them by name once, so it feels personalised.
function firstRunWelcome() {
  try {
    if (localStorage.getItem('prizm_welcomed') === '1') return;
    localStorage.setItem('prizm_welcomed', '1');
    const who = (S.family && S.member && S.member.name) ? S.member.name : (S.name || '').trim();
    const first = who && who !== 'Prizm User' ? who.split(' ')[0] : '';
    setTimeout(() => {
      try { toast(first ? `Welcome, ${first} · your ledger is ready` : 'Your ledger is ready'); } catch(e) {}
    }, 900);
  } catch(e) {}
}
...
const THEME_ACCENTS = { spectrum:'#6C5CE7', signature:'#C6A15B', minimal:'#9AA2FF', fintech:'#C6F24E', aurora:'#C4B5FD', graphite:'#BE9C5A', light:'#BE9C5A', dark:'#6C5CE7', noir:'#C9A15B' };
function applyTheme(mode, notify = false) {
  ...
  // Highlight the selected card in BOTH the Settings/Personalize picker (#theme-selector)
  // and the onboarding picker (#onb-theme) — the setup one was previously never updated.
  document.querySelectorAll('#theme-selector [data-theme-mode], #onb-theme [data-theme-mode]').forEach(btn => {
...
async function finishOnboarding(skip) {
  ...
  S.onboardAccts = [];
  // One last step before the app: offer to install to the home screen.
  try { showSetup('sp-install'); renderInstallStep(); }
  catch (e) { completeOnboarding(); }
}
// Ends onboarding for real.
function completeOnboarding() {
  if (S.onboardingNext === 'profilePicker') showProfilePicker();
  else enterApp();
}
function setOnboardAcctType(type, el) {
  const h = document.getElementById('onboard-acct-type');
  if (h) h.value = type;
  document.querySelectorAll('#onboard-type-grid .type-opt').forEach(o => o.classList.remove('active'));
  if (el) el.classList.add('active');
}

// ═══════════════════════════════════════════════════════════════════════════
//  INSTALL STEP · platform-aware "add to home screen" instructions
//  Chromium (Android/Windows/macOS/ChromeOS) fires beforeinstallprompt, so we
//  can show a real Install button. iOS and Safari never do — they need manual
//  Share-sheet steps, which is exactly where most PWAs lose people.
// ═══════════════════════════════════════════════════════════════════════════
function _platform() {
  ...
  const chromium = !firefox && (/chrome|chromium|crios/i.test(ua) || edge);
  // On iOS every browser is WebKit; only the share sheet can install.
  const safari = ios ? !chromeIOS && !/fxios|edgios/i.test(ua) : (/safari/i.test(ua) && !chromium && !firefox);
  return { ios, android, mac, win, edge, firefox, chromium, chromeIOS, safari };
}
function _installedAlready() {
  try {
...
function renderInstallStep() {
  const p = _platform();
  const icoEl = document.getElementById('install-os-ico');
  const osEl  = document.getElementById('install-os-name');
  const brEl  = document.getElementById('install-browser');
  const stEl  = document.getElementById('install-steps');
  const btn   = document.getElementById('install-do-btn');
  const done  = document.getElementById('install-done-note');
  const lede  = document.getElementById('install-lede');
  if (!stEl) return;

  // Already installed → nothing to do.
  if (_installedAlready()) {
    if (icoEl) icoEl.textContent = '✅';
    if (osEl)  osEl.textContent = 'Already installed';
    if (brEl)  brEl.textContent = 'You’re running the installed app';
    stEl.innerHTML = '';
    if (btn) btn.style.display = 'none';
    if (done) done.style.display = '';
    if (lede) lede.textContent = 'You’re all set — Prizm is already on your device.';
    return;
  }
  if (done) done.style.display = 'none';

  let ico = '📱', os = 'Your device', browser = '', steps = [], canPrompt = false;

  if (p.ios) {
    ico = '📱'; os = 'iPhone or iPad';
    browser = p.safari ? 'Safari' : (p.chromeIOS ? 'Chrome' : 'Your browser');
    steps = p.safari
      ? ['Tap the <b>Share</b> button at the bottom of Safari — the square with an arrow pointing up.',
         ...
         'Tap <b>Add</b> in the top right. Prizm appears on your home screen like any other app.']
      : ['Tap the <b>Share</b> button (the square with an arrow pointing up).',
         'Scroll down and tap <b>Add to Home Screen</b>.',
         'Tap <b>Add</b>. If you can’t find the option, open this page in <b>Safari</b> and try again.'];
  } else if (p.android) {
    ico = '🤖'; os = 'Android';
    if (_deferredInstallPrompt) {
      browser = 'Ready to install'; canPrompt = true;
      steps = ['Tap <b>Install Prizm</b> below.', 'Confirm <b>Install</b> in the popup.', 'Prizm lands in your app drawer and home screen.'];
    } else if (p.firefox) {
      browser = 'Firefox';
      steps = ['Open the <b>⋮</b> menu (top right).', 'Tap <b>Install</b> or <b>Add to Home screen</b>.', 'Confirm to add it.'];
    } else {
      browser = 'Chrome';
      steps = ['Open the <b>⋮</b> menu (top right).', 'Tap <b>Add to Home screen</b> or <b>Install app</b>.', 'Confirm to add it.'];
    }
  } else if (p.mac) {
    ico = '💻'; os = 'Mac';
    if (_deferredInstallPrompt) {
      browser = p.edge ? 'Edge' : 'Chrome'; canPrompt = true;
      steps = ['Click <b>Install Prizm</b> below.', 'Confirm <b>Install</b> in the dialog.', 'Prizm opens in its own window and lives in your Dock.'];
    } else if (p.safari) {
      browser = 'Safari';
      steps = ['In the menu bar, click <b>File</b>.', 'Choose <b>Add to Dock…</b>', 'Click <b>Add</b>. (Needs macOS Sonoma or newer — otherwise use Chrome or Edge.)'];
    } else if (p.firefox) {
      browser = 'Firefox';
      steps = ['Firefox on desktop can’t install web apps.', 'Open this page in <b>Chrome</b>, <b>Edge</b> or <b>Safari</b> to install it.', 'Or just bookmark it — Prizm works fine in a tab.'];
    } else {
      browser = 'Your browser';
      steps = ['Look for an <b>install icon</b> in the address bar (a screen with a down arrow).', 'Click it, then confirm <b>Install</b>.', 'Or use the browser menu → <b>Install Prizm</b>.'];
    }
  } else if (p.win) {
    ico = '🖥️'; os = 'Windows';
    if (_deferredInstallPrompt) {
      browser = p.edge ? 'Edge' : 'Chrome'; canPrompt = true;
      steps = ['Click <b>Install Prizm</b> below.', 'Confirm <b>Install</b> in the dialog.', 'Prizm gets its own window and a Start menu entry.'];
    } else if (p.firefox) {
      browser = 'Firefox';
      steps = ['Firefox on desktop can’t install web apps.', 'Open this page in <b>Edge</b> or <b>Chrome</b> to install it.', 'Or just bookmark it — Prizm works fine in a tab.'];
    } else {
      browser = p.edge ? 'Edge' : 'Chrome';
      steps = ['Look for the <b>install icon</b> in the address bar (a screen with a down arrow).', 'Click it, then confirm <b>Install</b>.', 'Or use the <b>…</b> menu → <b>Apps</b> → <b>Install this site as an app</b>.'];
    }
  } else {
    browser = 'Your browser';
    if (_deferredInstallPrompt) { canPrompt = true; steps = ['Tap <b>Install Prizm</b> below and confirm.']; }
    else steps = ['Open your browser menu.', 'Look for <b>Install app</b> or <b>Add to Home screen</b>.', 'Confirm to add it.'];
  }

  if (icoEl) icoEl.textContent = ico;
  if (osEl)  osEl.textContent = os;
  if (brEl)  brEl.textContent = browser;
  stEl.innerHTML = steps.map(s => '<li>' + s + '</li>').join('');
  if (btn) btn.style.display = canPrompt ? '' : 'none';
}
// Fires the native install prompt, then reflects the result in the step.
async function installFromSetup() {
  if (!_deferredInstallPrompt) { renderInstallStep(); return; }
  try {
    _deferredInstallPrompt.prompt();
    const res = await _deferredInstallPrompt.userChoice;
    ...
    if (res && res.outcome === 'accepted') {
      ...
      if (btn) btn.style.display = 'none';
      if (done) done.style.display = '';
      try { toast('✅ Prizm installed'); } catch (e) {}
    } else {
      renderInstallStep();
    }
  } catch (e) { renderInstallStep(); }
}
// If the browser announces installability while the user is on the step, update it live.
window.addEventListener('appinstalled', function () { try { renderInstallStep(); } catch (e) {} });
...
async function connectPersonal() {
  ...
  if (!_prizmSecret()) {
    const lockKey = _randomSecret();
    apiPost({ action: 'setSecret', secret: lockKey, current: '' });
    _setSecretLocal(lockKey);
  }
  startPolling();
...
function applyRemoteData(d) {
  ...
  S.accts = (d.accounts || []).map(a => ({ ...a, _open: Number(a.balance || 0) }));  // Sheet value = opening
  _reconcileBalances();
  S.members = d.members || [];
...
// ═══════════════════════════════════════════
const BETA_FEATURES = ['scribe', 'email', 'monthlyEmail', 'aiBoost', 'receipt'];
const BETA_LABELS = { scribe: 'Scribe', email: 'Email', monthlyEmail: 'Monthly email', aiBoost: 'AI Boost', receipt: 'Receipt scan' };
function _betaRead(k){ try { return localStorage.getItem(k) === '1'; } catch(e){ return false; } }
// Beta is BURIED: the Settings row does not exist until the user deliberately
// unlocks it by tapping the version number 7 times. A normal user will never see
// a trace of it, and turning Beta mode off re-buries it completely.
const BETA_UNLOCK_TAPS = 7;
let _betaTaps = 0, _betaTapTimer = null;
function betaUnlocked(){ return _betaRead('prizm_beta_unlocked'); }
function betaVersionTap(){
  clearTimeout(_betaTapTimer);
  _betaTapTimer = setTimeout(() => { _betaTaps = 0; }, 1200); // taps must be deliberate
  if (betaUnlocked()) return;                                  // already visible, nothing to do
  _betaTaps++;
  const left = BETA_UNLOCK_TAPS - _betaTaps;
  if (left <= 0) {
    _betaTaps = 0;
    try { localStorage.setItem('prizm_beta_unlocked', '1'); } catch(e){}
    renderBeta();
    try { toast('🧪 Beta features unlocked'); } catch(e){}
    try { if (navigator.vibrate) navigator.vibrate(18); } catch(e){}
  } else if (left <= 3) {
    try { toast(left + (left === 1 ? ' tap away' : ' taps away')); } catch(e){}
  }
}
...
function renderBeta(){
  if (!S.beta) return;
  // The Settings entry point only exists once unlocked. No unlock = no trace.
  const row = document.getElementById('settings-cat-beta');
  if (row) row.style.display = betaUnlocked() ? '' : 'none';
...
function toggleBetaMode(){
  ...
  if (!S.beta.on) {
    // Turning Beta OFF must leave ZERO trace for a normal user: switch every
    // feature off, undo their side effects, re-bury the Settings row, and back
    // out of the Beta page. Re-unlock with 7 taps on the version number.
    BETA_FEATURES.forEach(f => {
      S.beta[f] = false;
      try { localStorage.setItem('prizm_beta_' + f, '0'); } catch(e){}
    });
    try { setAddMode('form', false); } catch(e){}   // Scribe reverts to the classic form
    try { localStorage.removeItem('prizm_beta_unlocked'); } catch(e){}
    renderBeta();
    try { closeSettingsCat(); } catch(e){}
    toast('Beta mode off');
    return;
  }
  renderBeta();
...
function exportBackup() {
  try {
    ...
    try { localStorage.setItem('prizm_last_backup', String(Date.now())); } catch (e) {}
...
async function apiGet(skipIfUnchanged) {
  if (_gsMode()) {
    try {
      const d = await gsLoadAll();
      const txt = JSON.stringify(d);
      if (skipIfUnchanged && txt === _lastGetAllText) return null;
      _lastGetAllText = txt;
      return d;
    } catch (e) { console.warn('gsLoadAll:', e); return null; }
  }
...
function setupAIBoost(){
  if (aiKey()) {
    showCustomConfirm('Turn off AI Boost?', 'Quick add goes back to the built-in parser. Your Gemini key is only stored on this device — turning off simply forgets it here.', 'Turn off',
      () => { try { localStorage.removeItem('prizm_quick_ai_key'); } catch(e) {} _aiSub(); toast('AI Boost off'); });
    return;
  }
  openAiKeySheet();
}
// Themed in-app sheet to paste a free Gemini key (no cloud-platform scope,
// no auto-provisioning — keeps Prizm in Google's low-risk verification lane).
function openAiKeySheet(){
  const inp = document.getElementById('ai-key-input');
  if (inp) inp.value = '';
  const btn = document.getElementById('ai-key-save');
  if (btn) btn.disabled = true;
  openOverlay('ai-key-overlay');
}
function aiKeyInputChanged(el){
  const btn = document.getElementById('ai-key-save');
  if (btn) btn.disabled = !(el && el.value.trim().length > 20);
}
function saveAiKey(){
  const inp = document.getElementById('ai-key-input');
  const v = (inp ? inp.value : '').trim();
  if (v.length <= 20) { toast('That key looks too short — copy the whole thing'); return; }
  if (!/^AIza[0-9A-Za-z_\-]{30,}$/.test(v)) {
    // Not fatal — Google keys start AIza, but accept anything long in case the format changes.
    console.warn('AI key does not match expected AIza… pattern; saving anyway.');
  }
  try { localStorage.setItem('prizm_quick_ai_key', v); } catch(e) { toast('⚠️ Could not save the key on this device'); return; }
  _aiSub();
  closeOverlay('ai-key-overlay');
...
// (autoSetupAIKey removed — Prizm no longer provisions Cloud projects/keys.
//  Users paste a free Gemini key via openAiKeySheet(); see saveAiKey.)
...
// ── RECEIPT SCAN (Beta) · reads a receipt photo with the user's own Gemini key.
//    The image goes device → Google under the user's key; Prizm runs no servers.
function scanReceipt(){
  if (!betaOn('receipt')) { toast('Turn on Receipt scan in Settings › Beta'); return; }
  if (!aiKey()) { showCustomAlert('Receipt scan needs AI Boost', 'Receipt scanning reads the photo with your own free Gemini key. Turn on AI Boost first (Settings › Beta › AI Boost), then try again.'); return; }
  const inp = document.getElementById('receipt-file'); if (inp) { inp.value = ''; inp.click(); }
}
function _onReceiptFile(input){
  const f = input && input.files && input.files[0]; if (!f) return;
  const reader = new FileReader();
  reader.onload = async () => {
    const dataUrl = String(reader.result || '');
    const b64 = dataUrl.split(',')[1] || '';
    const mime = (dataUrl.match(/^data:([^;]+);/) || [,'image/jpeg'])[1];
    if (!b64) { toast('Could not read that image'); return; }
    try { loader(true); toast('📸 Reading your receipt…'); } catch(e){}
    const r = await _aiParseReceipt(b64, mime);
    try { loader(false); } catch(e){}
    if (!r || !(Number(r.total) > 0)) { toast('Couldn\'t read a total — try a clearer photo'); return; }
    try { closeOverlay('quick-overlay'); } catch(e){}
    const first = (S.accts && S.accts[0] && S.accts[0].name) || '';
    let cat; try { cat = guessCategory(r.merchant || '') || (getCatList(false).find(c=>c.n==='Other')||{n:'Other'}).n; } catch(e){ cat='Other'; }
    openScribeConfirm({ amount: Number(r.total), description: (r.merchant || 'Receipt'), type: 'expense', date: (/^\d{4}-\d{2}-\d{2}$/.test(r.date||'') ? r.date : todayISO()), category: cat, account: first });
  };
  reader.readAsDataURL(f);
}
async function _aiParseReceipt(b64, mime){
  const key = aiKey(); if (!key) return null;
  const prompt = 'You are reading a shop/restaurant receipt. Return ONLY JSON: {"merchant": string, "total": number, "date": "YYYY-MM-DD"}. "total" is the final amount paid (the grand total incl. tax), as a plain number. "merchant" is the shop/business name. "date" is the purchase date or empty if unclear.';
  try {
    ...
    const res = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + AI_MODEL + ':generateContent?key=' + encodeURIComponent(key), {
      ...
      body: JSON.stringify({ contents:[{ parts:[ {text:prompt}, {inlineData:{mimeType:mime, data:b64}} ] }], generationConfig:{ responseMimeType:'application/json', temperature:0 } })
    });
    clearTimeout(t);
    ...
    return { merchant: String(obj.merchant||'').trim(), total: Number(obj.total)||0, date: String(obj.date||'').slice(0,10) };
  } catch(e){ console.warn('receipt scan:', e); return null; }
}
...
const WEBSITE_URL = 'https://evildeveloper-app.github.io/marketingsite/';
function openPrizmWebsite() {
  if (!WEBSITE_URL) { toast('Website coming soon'); return; }
  try { window.open(WEBSITE_URL, '_blank'); } catch (e) {}
}
// ═══════════════════════════════════════════════════════════════════════════
//  PLANS · Free / Pro / Max, backed by Lemon Squeezy subscription licences
//
//  Prizm has no backend, so entitlement works like this:
//    1. User subscribes on the site → Lemon Squeezy emails them a licence key.
//    2. They paste it in → we ACTIVATE it against LS (binds it to this device).
//    3. Every LICENSE_RECHECK_DAYS we quietly RE-VALIDATE it online.
//    4. If they're offline we keep trusting the cached plan for LICENSE_GRACE_DAYS,
//       so a plane trip never locks anyone out. Past that, it falls back to Free.
//    5. If LS says expired/disabled/refunded, we downgrade immediately.
//
//  Swapping to Polar later: only _licensePost() and _tierFromLicense() change.
// ═══════════════════════════════════════════════════════════════════════════
const PRO_ENABLED = false;   // master switch — while false the app behaves as fully unlocked
const PRO_URL = '';          // Pro checkout URL
const MAX_URL = '';          // Max checkout URL
const LICENSE_API = 'https://api.lemonsqueezy.com/v1/licenses';
// Fill these with the Lemon Squeezy VARIANT IDs for each tier (numbers, as strings).
const LICENSE_TIERS = { pro: [], max: [] };
const LICENSE_RECHECK_DAYS = 7;   // re-verify online at most this often
const LICENSE_GRACE_DAYS  = 14;   // how long a cached plan survives with no successful check

const MEMBER_LIMITS = { free: 2, pro: 4, max: Infinity };
const FREE_MEMBER_LIMIT = MEMBER_LIMITS.free;   // kept: older call sites still reference it

function _lsGet(k){ try { return localStorage.getItem(k) || ''; } catch(e){ return ''; } }
function _lsSet(k,v){ try { localStorage.setItem(k,v); } catch(e){} }

// The single source of truth for entitlement. Never throws.
function currentPlan() {
  if (!PRO_ENABLED) return 'max';                       // pre-launch: everything unlocked
  try {
    if (_lsGet('prizm_pro') === '1') return 'pro';       // grandfathered legacy unlock codes
    const plan = _lsGet('prizm_plan') || 'free';
    if (plan !== 'pro' && plan !== 'max') return 'free';
    const status = _lsGet('prizm_license_status');
    if (status && status !== 'active') return 'free';    // expired / disabled / refunded
    const checked = parseInt(_lsGet('prizm_license_checked'), 10) || 0;
    if (checked && (Date.now() - checked) > LICENSE_GRACE_DAYS * 864e5) return 'free';
    return plan;
  } catch(e) { return 'free'; }
}
function isPro(){ const p = currentPlan(); return p === 'pro' || p === 'max'; }
function isMax(){ return currentPlan() === 'max'; }
function planLabel(){ return { free:'Free', pro:'Pro', max:'Max' }[currentPlan()] || 'Free'; }
function memberLimit(){ const l = MEMBER_LIMITS[currentPlan()]; return l == null ? MEMBER_LIMITS.free : l; }

// Map a Lemon Squeezy licence response onto one of our tiers.
function _tierFromLicense(d) {
  try {
    const vid = String(((d.meta || {}).variant_id) || '');
    const vname = String(((d.meta || {}).variant_name) || '').toLowerCase();
    if (LICENSE_TIERS.max.map(String).includes(vid) || /max/.test(vname)) return 'max';
    if (LICENSE_TIERS.pro.map(String).includes(vid) || /pro/.test(vname)) return 'pro';
  } catch(e) {}
  return 'pro'; // a valid key we can't classify still grants the lower paid tier
}
async function _licensePost(path, body) {
...
// Bind a licence key to this device.
async function activateLicense(key) {
  const d = await _licensePost('/activate', { license_key: key, instance_name: 'Prizm · ' + (navigator.platform || 'device') });
  if (!d || !d.activated) throw new Error((d && d.error) || 'That licence key was not accepted');
  _lsSet('prizm_license_key', key);
  _lsSet('prizm_license_instance', ((d.instance || {}).id) || '');
  _lsSet('prizm_plan', _tierFromLicense(d));
  _lsSet('prizm_license_status', 'active');
  _lsSet('prizm_license_checked', String(Date.now()));
  return currentPlan();
}
// Re-check an already-activated key. Silent by design: never interrupts the user.
async function validateLicense() {
  const key = _lsGet('prizm_license_key');
  if (!key) return;
  const d = await _licensePost('/validate', { license_key: key, instance_id: _lsGet('prizm_license_instance') });
  const status = ((d || {}).license_key || {}).status || (d && d.valid ? 'active' : 'inactive');
  _lsSet('prizm_license_status', status === 'active' ? 'active' : status);
  if (status === 'active') {
    _lsSet('prizm_plan', _tierFromLicense(d));
    _lsSet('prizm_license_checked', String(Date.now()));
  }
}
// Called on boot. Only hits the network when a check is actually due, and a
// failure (offline, LS down) is ignored — the grace window covers it.
function maybeRevalidateLicense() {
  if (!PRO_ENABLED) return;
  try {
    if (!_lsGet('prizm_license_key')) return;
    const checked = parseInt(_lsGet('prizm_license_checked'), 10) || 0;
    if (checked && (Date.now() - checked) < LICENSE_RECHECK_DAYS * 864e5) return;
    setTimeout(() => { validateLicense().catch(() => {}); }, 3000); // never blocks startup
  } catch(e) {}
}
...
function openProModal() { const c = document.getElementById('pro-code'); if (c) c.value = ''; _renderProModal(); openOverlay('pro-overlay'); }
function fakeSubscribe(tier){ toast('🎉 Enjoy ' + tier + ' free — payments aren\'t live yet'); try{ closeOverlay('pro-overlay'); }catch(e){} }
function _renderProModal() {
  const st = document.getElementById('pro-status');
  if (st) st.textContent = 'All features are free right now \u2728';
}
function getPro() { if (!PRO_ENABLED || !PRO_URL) return; try { window.open(PRO_URL, '_blank'); } catch (e) {} }
function getMax() { if (!PRO_ENABLED || !MAX_URL) return; try { window.open(MAX_URL, '_blank'); } catch (e) {} }
// Accepts a Lemon Squeezy subscription licence key OR a legacy offline OPUS code.
async function redeemProCode() {
  const el = document.getElementById('pro-code');
  const code = (el ? el.value : '').trim();
  if (!code) { toast('Paste your licence key first'); return; }
  // Legacy offline code path \u2014 instant, no network. Kept for comps and press keys.
  if (_proCodeValid(code)) {
    _lsSet('prizm_pro', '1');
  ...
  try {
    loader(true);
    const plan = await activateLicense(code);
    loader(false);
    toast('\ud83c\udf89 Prizm ' + (plan === 'max' ? 'Max' : 'Pro') + ' activated \u00b7 thank you!');
    _renderProModal(); try { renderSettings(); renderAll(); } catch (e) {}
    try { closeOverlay('pro-overlay'); } catch (e) {}
  } catch (e) {
    loader(false);
    showCustomAlert('Could not activate',
      (e && e.message ? e.message : 'That key was not accepted.') +
      '\n\nCheck the key in your confirmation email. If it is already active on too many devices, deactivate one first.');
  }
}
// Gate helpers for premium features (AI categorize, receipts, instant sync, etc.)
function requirePro() { if (isPro()) return true; openProModal(); return false; }
function requireMax() { if (isMax()) return true; openProModal(); return false; }
...
// Balances are DERIVED, never stored as a running total: display = opening + txSum.
// The Sheet's account "balance" column holds the OPENING balance; this recomputes
// the shown balance from it plus every transaction, so a sync can never wipe it.
function _reconcileBalances() {
  if (S.mode !== 'sheets') return;
  (S.accts || []).forEach(a => {
    if (a._open == null || isNaN(Number(a._open))) a._open = Number(a.balance || 0) - _accountTxSum(a.name);
    a.balance = Math.round(((Number(a._open) || 0) + _accountTxSum(a.name)) * 100) / 100;
  });
}
function recalcBalances() {
  ...
  saveLocalData();
  renderAll();
  toast('✓ Balances recomputed from your transactions');
}

function handleLocalPost(b) {
  if (b.action === 'addTransaction') {
    const id = b.id || 'tx_' + Date.now() + '_' + Math.random().toString(36).slice(2,6);
  ...
  if (b.action === 'addAccount') { S.accts.push({ id: b.id || 'ac_'+Date.now(), name: b.name, type: b.type, balance: Number(b.balance)||0, _open: Number(b.balance)||0 }); saveLocalData(); return {success:true}; }
  if (b.action === 'updateAccount') {
    ...
    if (a) {
      ...
      if (S.mode === 'sheets') { a._open = Number(b.balance)||0; a.balance = Math.round(((a._open) + _accountTxSum(a.name)) * 100) / 100; }
      else { a.balance = Number(b.balance)||0; }
      saveLocalData();
    }
    return {success:true};
  }
  ...
  if (b.action === 'setBudget') {
    const id = b.id || 'bg_' + Date.now() + '_' + Math.random().toString(36).slice(2,5);
...
async function outboxFlush() {
  ...
  try {
    ...
    while (q.length) {
      ...
      if (_gsMode()) {
        try {
          const d = await gsApplyAction(item.body);
          if (d && d.error) { console.error('Outbox: Sheets API rejected a write', item, d.error); _outboxDeadLetter(item, d.error); }
...
function advanceDate(dateStr, freq, targetDay) {
  ...
  if (freq === 'weekly') {
    const dow = new Date(Y, M, D).getDay();       // the weekday this repeats on
    const nd = new Date(Y, M, D + 7);
    while (nd.getDay() !== dow) nd.setDate(nd.getDate() + 1); // guard against any drift
    return isoLocal(nd);
  }
  if (freq === 'yearly') { Y += 1; }
...
// Fast-forward recurrences that fell behind (missed opens, stale sync) to their
// correct upcoming date, so the schedule never drifts or piles up silently.
function healRecurring() {
  try {
    const today = todayISO();
    const perDays = { daily:1, weekly:7, monthly:31, yearly:366 };
    let changed = false;
    (S.recurring || []).forEach(r => {
      if (r.active === false || !r.nextDate) return;
      const freq = String(r.frequency || 'monthly').toLowerCase();
      const per = perDays[freq] || 31;
      let nd = String(r.nextDate).slice(0,10);
      // Only heal items that are MORE than two full periods overdue (truly stale);
      // a normally-due item is left for the due popup.
      const behind = Math.round((dateMs(today) - dateMs(nd)) / 86400000);
      if (behind <= per * 2) return;
      const tday = r.targetDay || Number(nd.slice(8,10)) || undefined;
      let guard = 0;
      while (nd < today && guard < 800) { nd = advanceDate(nd, freq, tday); guard++; }
      if (nd !== String(r.nextDate).slice(0,10)) {
        r.nextDate = nd; changed = true;
        try { apiPost({ action:'updateRecurring', id:r.id, name:r.name, amount:r.amount, type:r.type, category:r.category, account:r.account, frequency:r.frequency, nextDate:nd, targetDay:tday, memberId:r.memberId, active:true }); } catch(e){}
      }
    });
    if (changed) { try { saveLocalData(); } catch(e){} }
  } catch(e) { console.warn('healRecurring:', e); }
}
...
// Split a formatted amount into <symbol><number> so the currency glyph can be
// dimmed and the number set heavy (Prizm two-weight numerals). Returns HTML.
function fmtMoneyParts(amount, signPrefix){
  const full = fmtMoney(amount);
  const m = full.match(/^([^0-9\-]*)(.*)$/);   // leading currency symbol vs the rest
  const sym = m ? m[1] : (S.cur.sym || '$');
  const num = m ? m[2] : full;
  const sign = signPrefix || '';
  return '<span class="amt-sign">' + esc(sign) + '</span><span class="amt-sym">' + esc(sym) + '</span><span class="amt-num">' + esc(num) + '</span>';
}
...
// ── MERCHANT DIRECTORY · ~160 of Australia's most common brands ────────────────
// We store ONLY the brand's official domain + match keywords (factual data). The
// app fetches each brand's real favicon at runtime for identification; no logo
// artwork is bundled. Unknown/offline → falls back to the prism-gem category icon.
const MERCHANTS = [
  ['woolworths.com.au', 'woolworths', 'woolies'],
  ['coles.com.au', 'coles'],
  ['aldi.com.au', 'aldi'],
  ['iga.com.au', 'iga'],
  ['costco.com.au', 'costco'],
  ['harrisfarm.com.au', 'harris farm'],
  ['foodland.com.au', 'foodland'],
  ['drakes.com.au', 'drakes'],
  ['spudshed.com.au', 'spudshed'],
  ['amazonfresh.com', 'amazon fresh'],
  ['milkrun.com', 'milkrun'],
  ['bp.com', 'bp'],
  ['shell.com.au', 'shell'],
  ['ampol.com.au', 'ampol'],
  ['caltex.com.au', 'caltex'],
  ['7eleven.com.au', '7 eleven', '7eleven', 'seven eleven'],
  ['unitedpetroleum.com.au', 'united petroleum'],
  ['mobil.com.au', 'mobil'],
  ['otr.com.au', 'otr'],
  ['libertyoil.com.au', 'liberty oil'],
  ['metropetroleum.com.au', 'metro petroleum'],
  ['pumaenergy.com', 'puma energy'],
  ['mcdonalds.com.au', 'mcdonalds', 'maccas', 'mcdonald'],
  ['kfc.com.au', 'kfc'],
  ['hungryjacks.com.au', 'hungry jacks', 'hungryjacks'],
  ['dominos.com.au', 'dominos'],
  ['pizzahut.com.au', 'pizza hut'],
  ['crust.com.au', 'crust'],
  ['subway.com.au', 'subway'],
  ['nandos.com.au', 'nandos'],
  ['redrooster.com.au', 'red rooster'],
  ['oporto.com.au', 'oporto'],
  ['grilld.com.au', 'grilld'],
  ['guzmanygomez.com.au', 'guzman', 'gyg'],
  ['zambrero.com.au', 'zambrero'],
  ['madmex.com.au', 'mad mex'],
  ['salsas.com.au', 'salsas'],
  ['bettysburgers.com.au', 'bettys burgers'],
  ['sushihub.com.au', 'sushi hub'],
  ['sushisushi.com.au', 'sushi sushi'],
  ['rolld.com.au', 'rolld'],
  ['schnitz.com.au', 'schnitz'],
  ['soulorigin.com.au', 'soul origin'],
  ['sumosalad.com', 'sumo salad'],
  ['zeusstreetgreek.com.au', 'zeus street greek'],
  ['tacobell.com.au', 'taco bell'],
  ['carls.com.au', 'carls jr'],
  ['wendys.com', 'wendys'],
  ['popeyes.com', 'popeyes'],
  ['chipotle.com', 'chipotle'],
  ['fiveguys.com.au', 'five guys'],
  ['shakeshack.com', 'shake shack'],
  ['krispykreme.com.au', 'krispy kreme'],
  ['dunkindonuts.com', 'dunkin'],
  ['timhortons.com', 'tim hortons'],
  ['donutking.com.au', 'donut king'],
  ['bettysburgers.com.au', 'bettys'],
  ['starbucks.com.au', 'starbucks'],
  ['gloriajeanscoffees.com', 'gloria jeans'],
  ['coffeeclub.com.au', 'coffee club'],
  ['zarraffas.com', 'zarraffas'],
  ['muffinbreak.com.au', 'muffin break'],
  ['michels.com.au', 'michels'],
  ['jamaicablue.com.au', 'jamaica blue'],
  ['degani.com.au', 'degani'],
  ['boostjuice.com.au', 'boost juice'],
  ['chatime.com.au', 'chatime'],
  ['gongcha.com.au', 'gong cha'],
  ['sanchurro.com', 'san churro'],
  ['bakersdelight.com.au', 'bakers delight'],
  ['brumbys.com.au', 'brumbys'],
  ['ubereats.com', 'uber eats', 'ubereats'],
  ['doordash.com', 'doordash'],
  ['menulog.com.au', 'menulog'],
  ['deliveroo.com.au', 'deliveroo'],
  ['hellofresh.com.au', 'hello fresh', 'hellofresh'],
  ['marleyspoon.com.au', 'marley spoon'],
  ['youfoodz.com', 'youfoodz'],
  ['dinnerly.com.au', 'dinnerly'],
  ['everyplate.com.au', 'everyplate'],
  ['liteneasy.com.au', 'lite n easy'],
  ['mymusclechef.com', 'my muscle chef'],
  ['uber.com', 'uber'],
  ['didiglobal.com', 'didi'],
  ['ola.com.au', 'ola'],
  ['bolt.eu', 'bolt'],
  ...
  ['neuron.sg', 'neuron'],
  ['ridebeam.com', 'beam'],
  ['transportnsw.info', 'opal'],
  ['ptv.vic.gov.au', 'myki'],
  ['translink.com.au', 'translink', 'go card'],
  ['13cabs.com.au', '13cabs'],
  ['cabcharge.com.au', 'cabcharge'],
  ['linkt.com.au', 'linkt', 'toll'],
  ['wilsonparking.com.au', 'wilson parking'],
  ['secureparking.com.au', 'secure parking'],
  ['easypark.com', 'easypark'],
  ['cellopark.com.au', 'cellopark'],
  ['qantas.com', 'qantas'],
  ['jetstar.com', 'jetstar'],
  ['virginaustralia.com', 'virgin australia'],
  ['rex.com.au', 'rex airlines'],
  ['flybonza.com', 'bonza'],
  ['airnewzealand.com.au', 'air new zealand'],
  ['singaporeair.com', 'singapore airlines'],
  ['emirates.com', 'emirates'],
  ['etihad.com', 'etihad'],
  ['qatarairways.com', 'qatar airways'],
  ['cathaypacific.com', 'cathay'],
  ...
  ['trivago.com.au', 'trivago'],
  ['wotif.com', 'wotif'],
  ['webjet.com.au', 'webjet'],
  ['flightcentre.com.au', 'flight centre'],
  ['trip.com', 'trip com'],
  ['skyscanner.com.au', 'skyscanner'],
  ['klook.com', 'klook'],
  ['kayak.com.au', 'kayak'],
  ...
  ['stan.com.au', 'stan'],
  ['binge.com.au', 'binge'],
  ['kayosports.com.au', 'kayo'],
  ['foxtel.com.au', 'foxtel'],
  ['paramountplus.com', 'paramount'],
  ['primevideo.com', 'prime video', 'amazon prime'],
  ...
  ['sbs.com.au', 'sbs on demand'],
  ['abc.net.au', 'abc iview', 'iview'],
  ['9now.com.au', '9now'],
  ['7plus.com.au', '7plus'],
  ['10play.com.au', '10 play'],
  ['spotify.com', 'spotify'],
  ...
  ['audible.com.au', 'audible'],
  ['siriusxm.com', 'siriusxm'],
  ['apple.com', 'apple'],
  ['icloud.com', 'icloud'],
  ['one.google.com', 'google one'],
  ['microsoft.com', 'microsoft', 'office 365', 'microsoft 365'],
  ['adobe.com', 'adobe'],
  ...
  ['reddit.com', 'reddit'],
  ['discord.com', 'discord'],
  ['playstation.com', 'playstation', 'psn'],
  ['xbox.com', 'xbox', 'game pass'],
  ['nintendo.com.au', 'nintendo'],
  ...
  ['jbhifi.com.au', 'jb hi-fi', 'jbhifi'],
  ['harveynorman.com.au', 'harvey norman'],
  ['thegoodguys.com.au', 'good guys'],
  ['officeworks.com.au', 'officeworks'],
  ['bunnings.com.au', 'bunnings'],
  ['ikea.com', 'ikea'],
  ['bestbuy.com', 'best buy'],
  ['walmart.com', 'walmart'],
  ['amartfurniture.com.au', 'amart'],
  ['freedom.com.au', 'freedom'],
  ['fantasticfurniture.com.au', 'fantastic furniture'],
  ['nickscali.com.au', 'nick scali'],
  ['adairs.com.au', 'adairs'],
  ['pillowtalk.com.au', 'pillow talk'],
  ['spotlightstores.com', 'spotlight'],
  ['domayne.com.au', 'domayne'],
  ['fortywinks.com.au', 'forty winks'],
  ['snooze.com.au', 'snooze'],
  ['beaconlighting.com.au', 'beacon lighting'],
  ['mitre10.com.au', 'mitre 10'],
  ['totaltools.com.au', 'total tools'],
  ['sydneytools.com.au', 'sydney tools'],
  ['stratco.com.au', 'stratco'],
  ...
  ['thenile.com.au', 'the nile'],
  ['theiconic.com.au', 'the iconic'],
  ['asos.com', 'asos'],
  ['boohoo.com', 'boohoo'],
  ['princesspolly.com.au', 'princess polly'],
  ['cottonon.com', 'cotton on'],
  ['uniqlo.com', 'uniqlo'],
  ['zara.com', 'zara'],
  ['hm.com', 'h m', 'h&m'],
  ['nike.com', 'nike'],
  ['adidas.com.au', 'adidas'],
  ['puma.com', 'puma'],
  ['newbalance.com.au', 'new balance'],
  ['underarmour.com.au', 'under armour'],
  ['lululemon.com.au', 'lululemon'],
  ['gymshark.com', 'gymshark'],
  ['jdsports.com.au', 'jd sports'],
  ['rebelsport.com.au', 'rebel sport'],
  ['footlocker.com.au', 'foot locker'],
  ['culturekings.com.au', 'culture kings'],
  ['universalstore.com', 'universal store'],
  ['generalpants.com', 'general pants'],
  ['gluestore.com.au', 'glue store'],
  ['hypedc.com', 'hype dc'],
  ['platypusshoes.com.au', 'platypus'],
  ['lornajane.com.au', 'lorna jane'],
  ['decathlon.com.au', 'decathlon'],
  ['country road.com.au', 'country road'],
  ['cottonon.com', 'cotton on'],
  ['anacondastores.com', 'anaconda'],
  ['bcf.com.au', 'bcf'],
  ['kathmandu.com.au', 'kathmandu'],
  ['macpac.com.au', 'macpac'],
  ['snowys.com.au', 'snowys'],
  ['wildearth.com.au', 'wild earth'],
  ['chemistwarehouse.com.au', 'chemist warehouse'],
  ['priceline.com.au', 'priceline'],
  ['terrywhitechemmart.com.au', 'terry white'],
  ['amcal.com.au', 'amcal'],
  ['bloomsthechemist.com.au', 'blooms'],
  ['mecca.com.au', 'mecca'],
  ['sephora.com.au', 'sephora'],
  ['adorebeauty.com.au', 'adore beauty'],
  ['nationalpharmacies.com.au', 'national pharmacies'],
  ['petbarn.com.au', 'petbarn'],
  ['petstock.com.au', 'petstock'],
  ['petcircle.com.au', 'pet circle'],
  ['budgetpetproducts.com.au', 'budget pet products'],
  ['rspca.org.au', 'rspca'],
  ['booktopia.com.au', 'booktopia'],
  ['dymocks.com.au', 'dymocks'],
  ['qbd.com.au', 'qbd'],
  ['readings.com.au', 'readings'],
  ['telstra.com.au', 'telstra'],
  ['optus.com.au', 'optus'],
  ['vodafone.com.au', 'vodafone'],
  ['tpg.com.au', 'tpg'],
  ['belong.com.au', 'belong'],
  ['aussiebroadband.com.au', 'aussie broadband'],
  ['iinet.net.au', 'iinet'],
  ['amaysim.com.au', 'amaysim'],
  ['boost.com.au', 'boost mobile'],
  ['felixmobile.com.au', 'felix mobile'],
  ['superloop.com', 'superloop'],
  ['dodo.com', 'dodo'],
  ['koganmobile.com.au', 'kogan mobile'],
  ['moosemobile.com.au', 'moose mobile'],
  ['tangerine.com.au', 'tangerine'],
  ['circles.life', 'circles life'],
  ['lebara.com.au', 'lebara'],
  ['spintel.net.au', 'spintel'],
  ['exetel.com.au', 'exetel'],
  ['southernphone.com.au', 'southern phone'],
  ['commbank.com.au', 'commbank', 'commonwealth bank'],
  ...
  ['ubank.com.au', 'ubank'],
  ['up.com.au', 'up bank'],
  ['stgeorge.com.au', 'st george'],
  ['bankofmelbourne.com.au', 'bank of melbourne'],
  ['banksa.com.au', 'banksa'],
  ['mebank.com.au', 'me bank'],
  ['greatsouthernbank.com.au', 'great southern bank'],
  ['judo.bank', 'judo bank'],
  ['revolut.com', 'revolut'],
  ['wise.com', 'wise'],
  ['paypal.com', 'paypal'],
  ['afterpay.com', 'afterpay'],
  ['zip.co', 'zip pay', 'zippay'],
  ['klarna.com', 'klarna'],
  ['humm.com.au', 'humm'],
  ['latitudefinancial.com.au', 'latitude'],
  ['beforepay.com.au', 'beforepay'],
  ['stripe.com', 'stripe'],
  ['squareup.com', 'square'],
  ...
  ['simplyenergy.com.au', 'simply energy'],
  ['momentumenergy.com.au', 'momentum energy'],
  ['powershop.com.au', 'powershop'],
  ['lumoenergy.com.au', 'lumo'],
  ['globirdenergy.com.au', 'globird'],
  ['tangoenergy.com', 'tango energy'],
  ['actewagl.com.au', 'actewagl'],
  ['sydneywater.com.au', 'sydney water'],
  ['sawater.com.au', 'sa water'],
  ['yvw.com.au', 'yarra valley water'],
  ['nrma.com.au', 'nrma'],
  ['racv.com.au', 'racv'],
  ['racq.com.au', 'racq'],
  ['raa.com.au', 'raa'],
  ['rac.com.au', 'rac'],
  ['aami.com.au', 'aami'],
  ['bingle.com.au', 'bingle'],
  ['youi.com.au', 'youi'],
  ['allianz.com.au', 'allianz'],
  ['qbe.com', 'qbe'],
  ['budgetdirect.com.au', 'budget direct'],
  ['bupa.com.au', 'bupa'],
  ...
  ['gmhba.com.au', 'gmhba'],
  ['ahm.com.au', 'ahm'],
  ['frankhealthinsurance.com.au', 'frank'],
  ['hbf.com.au', 'hbf'],
  ['australianunity.com.au', 'australian unity'],
  ['danmurphys.com.au', 'dan murphys'],
  ['bws.com.au', 'bws'],
  ['liquorland.com.au', 'liquorland'],
  ['firstchoiceliquor.com.au', 'first choice'],
  ['cellarbrations.com.au', 'cellarbrations'],
  ['vintagecellars.com.au', 'vintage cellars'],
  ['jimmybrings.com.au', 'jimmy brings'],
  ['anytimefitness.com.au', 'anytime fitness'],
  ['fitnessfirst.com.au', 'fitness first'],
  ['jetts.com.au', 'jetts'],
  ['f45training.com', 'f45'],
  ['goodlifehealthclubs.com.au', 'goodlife'],
  ['snapfitness.com', 'snap fitness'],
  ['plusfitness.com.au', 'plus fitness'],
  ['crunch.com.au', 'crunch'],
  ['worldgym.com.au', 'world gym'],
  ['clublime.com.au', 'club lime'],
  ['ato.gov.au', 'ato', 'australian tax'],
  ['medicare.gov.au', 'medicare'],
  ['servicesaustralia.gov.au', 'centrelink'],
  ['my.gov.au', 'mygov'],
  ['service.nsw.gov.au', 'service nsw'],
  ['vic.gov.au', 'service victoria'],
  ['vicroads.vic.gov.au', 'vicroads'],
  ['auspost.com.au', 'australia post', 'auspost'],
  ['startrack.com.au', 'startrack'],
  ['transurban.com', 'transurban'],
  ['samsung.com', 'samsung'],
  ['dell.com', 'dell'],
  ['hp.com', 'hp'],
  ['lenovo.com', 'lenovo'],
  ...
  ['pccasegear.com', 'pccasegear'],
  ['mwave.com.au', 'mwave'],
  ['smiggle.com.au', 'smiggle'],
  ['lego.com', 'lego'],
  ['bigw.com.au', 'big w'],
  ['eb games.com.au', 'eb games'],
  ['gamestop.com', 'gamestop'],
];
function merchantFor(desc){
  const n = ' ' + String(desc||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim() + ' ';
  let best = null;
  for (let i=0;i<MERCHANTS.length;i++){
    const m = MERCHANTS[i];
    for (let j=1;j<m.length;j++){ const kw = m[j].replace(/[^a-z0-9]+/g,' ').trim(); if (n.indexOf(' '+kw+' ') !== -1){ if (!best || kw.length > best.keyword.length) best = { domain:m[0], keyword:kw }; } }
  }
  return best;
}
// Icon for a transaction: real favicon for known merchants (white tile), else the
// prism-gem category icon. Favicon failure quietly removes the img → gem shows.
function merchantIconHTML(desc, cat, isInc){
  const emo = getCatIcon(cat, isInc);
  const col = getCatColor(cat), edge = getCatEdge(cat);
  const m = merchantFor(desc);
  if (m){
    const fav = 'https://www.google.com/s2/favicons?sz=128&domain=' + encodeURIComponent(m.domain);
    return `<div class="tx-ico brand" style="background:${col};--gem-edge:${edge}"><span class="tx-emo">${emo}</span><img class="tx-fav" src="${fav}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()"></div>`;
  }
  return `<div class="tx-ico gem" style="background:${col};--gem-edge:${edge}">${emo}</div>`;
}

// Amount hint fades once the user starts typing a value.
function _addAmtHint(){
  const h = document.getElementById('add-amt-hint');
  const v = (document.getElementById('add-amt')||{}).value || '';
  if (h) h.style.opacity = v.trim() ? '0' : '1';
}
const _ADD_PENCIL = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';
// Live merchant icon in the description row: real favicon for a known brand, else pencil.
function updateAddDescIcon(){
  const inp = document.getElementById('add-desc');
  const box = document.getElementById('add-desc-ico');
  if (!inp || !box) return;
  const m = (typeof merchantFor === 'function') ? merchantFor(inp.value) : null;
  if (m){
    box.style.background = '#fff';
    box.innerHTML = '<img src="https://www.google.com/s2/favicons?sz=64&domain=' + encodeURIComponent(m.domain) + '" alt="" referrerpolicy="no-referrer" onerror="this.parentNode.style.background=\'\';this.parentNode.innerHTML=_ADD_PENCIL">';
  } else {
    box.style.background = '';
    box.innerHTML = _ADD_PENCIL;
  }
}

function getCatHue(name) {
  const n = baseCat(String(name || '')).toLowerCase();
  if (!n) return 45;
  let h = 0; for (let i = 0; i < n.length; i++) h = (h * 31 + n.charCodeAt(i)) >>> 0;
  return h % 360;
}
function getCatColor(name) {
  // Prism metaphor: each category catches light at its own hue. Muted, low-alpha
  // jewel tint (never a loud pastel), deterministic from the name so it's stable.
  const n = baseCat(String(name || '')).toLowerCase();
  if (!n) return 'var(--accent-soft)';
  let h = 0; for (let i = 0; i < n.length; i++) h = (h * 31 + n.charCodeAt(i)) >>> 0;
  const hue = h % 360;
  return 'hsla(' + hue + ',48%,58%,0.20)';
}
// Slightly stronger version of the same hue for the gem's facet edge.
function getCatEdge(name) {
  const n = baseCat(String(name || '')).toLowerCase();
  if (!n) return 'var(--accent)';
  let h = 0; for (let i = 0; i < n.length; i++) h = (h * 31 + n.charCodeAt(i)) >>> 0;
  return 'hsla(' + (h % 360) + ',52%,62%,0.55)';
}
...
// Smoothly tween a number element from its last shown value to a new one.
function _countUp(el, to, fmtFn, onFrame) {
  if (!el) return;
  const start = (typeof el._cuv === 'number') ? el._cuv : (to * 0.72);
  el._cuv = to;
  if (Math.abs(start - to) < 0.005) { el.innerHTML = fmtFn(to); if (onFrame) onFrame(el); return; }
  const t0 = performance.now(), dur = 620;
  function step(now) {
    let p = Math.min(1, (now - t0) / dur); p = 1 - Math.pow(1 - p, 3);
    el.innerHTML = fmtFn(start + (to - start) * p);
    if (onFrame) onFrame(el);
    if (p < 1) requestAnimationFrame(step); else { el.innerHTML = fmtFn(to); if (onFrame) onFrame(el); }
  }
...
function celebrateAdd() {
  try {
    ...
    // Draw the tick the natural way: the short leg first, then the long stroke.
    n.innerHTML = '<span class="pz-burst"></span><svg viewBox="0 0 24 24"><path d="M4 12l5 5L20 6"/></svg>';
    document.body.appendChild(n);
    if (navigator.vibrate) { try { navigator.vibrate([10,30,14]); } catch(e){} }
    setTimeout(() => { try { n.remove(); } catch(e){} }, 1050);
  } catch(e) {}
}
...
function quickAddStar(i) {
  ...
  apiPost({ action: 'addTransaction', id: 'tx_' + Date.now() + '_' + Math.random().toString(36).slice(2,6), ts: Date.now(), type: s.type || 'expense', amount: s.amount, category: s.category, description: s.description, account: s.account || (S.accts[0] && S.accts[0].name) || '', date: todayISO(), notes: '', memberId: S.member ? S.member.id : null });
...
// Plain-language insight line under the hero. Key facts are wrapped in <b> which
// the CSS threads in gold — Ducat's "talk to me like a person" grammar, Prizm's voice.
function _renderHomeSentence(spent, income, showIncome, periodStart){
  const el = document.getElementById('home-sentence');
  if (!el) return;
  const startOfToday = new Date(); startOfToday.setHours(0,0,0,0);
  let todaySpent = 0;
  (S.txs || []).forEach(t => { if (t.type !== 'income' && dateMs(t.date) >= startOfToday.getTime()) todaySpent += (parseFloat(t.amount)||0); });

  // Upcoming recurring within the next 7 days.
  const today = new Date(); today.setHours(0,0,0,0);
  const in7 = today.getTime() + 7*86400000;
  let upcoming = 0, upcomingCount = 0;
  (S.recurring || []).forEach(r => {
    if (r.active === false || !r.nextDate) return;
    const d = dateMs(String(r.nextDate).slice(0,10));
    if (d >= today.getTime() && d <= in7 && r.type !== 'income') { upcoming += (parseFloat(r.amount)||0); upcomingCount++; }
  });

  const g = v => '<b>' + esc(fmtMoney(v)) + '</b>';
  let parts = [];
  if (todaySpent > 0) parts.push('You\u2019ve spent ' + g(todaySpent) + ' today');
  else parts.push('Nothing spent yet today');

  if (upcomingCount > 0) {
    parts.push('and have ' + g(upcoming) + ' in ' + (upcomingCount === 1 ? 'one payment' : '<b>' + upcomingCount + ' payments</b>') + ' coming up this week');
  } else {
    parts.push('and no payments due this week');
  }
...
function renderHome() {
  ...
  // No stat row when income is off → drop the hero's bottom "chin".
  const _hh = document.getElementById('home-hero');
  if (_hh) _hh.classList.toggle('lean', !showIncome);
  _countUp(document.getElementById("hero-balance"), heroTarget, v => fmtMoney(v), el => fitHeroAmount(el, 44, 26));
...
function txHTML(t) {
  ...
  return `
    <div class="tx-item" onclick="openEditTx('${esc(t.id)}')">
      ${merchantIconHTML(t.description || t.category, t.category, isInc)}
      <div class="tx-body">
      ...
      <div class="tx-right">
        <div class="tx-amt ${colorClass}">${fmtMoneyParts(t.amount, isInc ? '+' : '-')}</div>
      </div>
    </div>`;
}
// Day-key + long label for grouping the ledger into breathing day-sections with
// a single gold spine date in the margin.
function _txDayKey(d){ const x = parseDate(d); return isNaN(x) ? String(d||'') : (x.getFullYear()+'-'+x.getMonth()+'-'+x.getDate()); }
function _txDayLabel(d){ const x = parseDate(d); return isNaN(x) ? esc(String(d||'')) : x.toLocaleDateString('en-US',{month:'short'})+' '+String(x.getDate()).padStart(2,'0'); }
...
function renderTxList(arr, containerId, isEmptyCompact = false) {
  ...
  // Group consecutive entries by day. Each day becomes a row: a quiet gold "spine"
  // date in the left margin + the day's stacked transactions (hairline-separated).
  let html = '', curKey = null, buf = [];
  const flush = () => {
    if (!buf.length) return;
    html += `<div class="tx-day"><div class="tx-spine" aria-hidden="true"><span>${_txDayLabel(buf[0].date)}</span></div><div class="tx-day-items">${buf.map(t => txHTML(t)).join('')}</div></div>`;
    buf = [];
  };
  arr.forEach(t => { const k = _txDayKey(t.date); if (k !== curKey) { flush(); curKey = k; } buf.push(t); });
  flush();
...
// Activity value strip: totals + a plain sentence over the currently filtered view.
function _renderActivitySummary(list) {
  const box = document.getElementById('act-summary');
  if (!box) return;
  if (!list || !list.length) { box.style.display = 'none'; return; }
  let out = 0, inc = 0;
  list.forEach(t => { const a = Number(t.amount) || 0; if (t.type === 'income') inc += a; else out += a; });
  const net = inc - out;
  const setP = (id, html) => { const e = document.getElementById(id); if (e) e.innerHTML = html; };
  const money = v => { const m = fmtMoney(v); const x = m.match(/^([^0-9\-]*)(.*)$/); return '<span class="ass-sym">' + esc(x?x[1]:'') + '</span>' + esc(x?x[2]:m); };
  setP('act-sum-out', money(out));
  setP('act-sum-in', money(inc));
  const netEl = document.getElementById('act-sum-net');
  if (netEl) { netEl.innerHTML = (net < 0 ? '<span class="ass-sym">- </span>' : '') + money(Math.abs(net)); netEl.classList.toggle('income', net >= 0); }
  const scope = (S.filter && S.filter !== 'all') ? esc(String(S.filter)) : 'all activity';
  const line = document.getElementById('act-sum-line');
  if (line) {
    line.innerHTML = (S.filter === 'income')
      ? `<b>${esc(fmtMoney(inc))}</b> came in across <b>${list.length}</b> ${list.length===1?'entry':'entries'}.`
      : `You've spent <b>${esc(fmtMoney(out))}</b> across <b>${list.length}</b> ${list.length===1?'entry':'entries'}${scope==='all activity'?'':' in '+scope}.`;
  }
...
function renderAnalytics() {
  ...
  // Ducat-style hero: one big number + a plain sentence with gold-threaded facts.
  try {
    const heroAmt = document.getElementById('an-hero-amt');
    const heroLbl = document.getElementById('an-hero-label');
    const heroSen = document.getElementById('an-hero-sentence');
    if (heroAmt) { heroAmt.innerHTML = fmtMoneyParts(-tExp); fitHeroAmount(heroAmt, 52, 26); }
    if (heroLbl) heroLbl.textContent = 'Spent · ' + lbl;
    // vs previous equal-length window (down = good, shown green; up = red)
    try {
      const dEl = document.getElementById('an-hero-delta');
      if (dEl) {
        const nowMs = Date.now();
        const startMs = (typeof cutOff === 'number' && cutOff > 0) ? cutOff : (nowMs - 30*864e5);
        const len = Math.max(864e5, nowMs - startMs);
        const prevStart = startMs - len;
        let prevExp = 0;
        (S.txs || []).forEach(t => { const ms = dateMs(t.date); if (ms >= prevStart && ms < startMs && t.type !== 'income') prevExp += (parseFloat(t.amount)||0); });
        if (prevExp > 0) {
          const pct = Math.round(((tExp - prevExp) / prevExp) * 100);
          const up = pct > 0;
          const arrow = pct === 0 ? '' : (up ? '▲' : '▼');
          const cls = pct === 0 ? 'flat' : (up ? 'up' : 'down');
          dEl.className = 'an-hero-delta ' + cls;
          dEl.innerHTML = arrow + ' ' + Math.abs(pct) + '% vs last ' + (lbl.toLowerCase().indexOf('day')>=0?'week':(lbl.toLowerCase().indexOf('year')>=0?'year':'period'));
          dEl.style.display = '';
    ...
    if (heroSen) {
      ...
      const nCats = Object.keys(catTotals).length;
      let sen = 'Across <b>' + nCats + '</b> ' + (nCats===1?'category':'categories');
      if (topCat) sen += ', most on <b>' + esc(topCat) + '</b> (<b>' + esc(fmtMoney(catTotals[topCat])) + '</b>)';
      if (tInc > 0) sen += '. You brought in <b>' + esc(fmtMoney(tInc)) + '</b>';
      heroSen.innerHTML = sen + '.';
    }
  } catch(e) { console.warn('an-hero:', e); }
...
function renderCatBarsInto(wrap, catObj, total, limit, isIncome) {
  ...
  wrap.innerHTML = arr.map(c => {
    ...
    const _h = getCatHue(c.cat);
    const _grad = isIncome ? '' : `background:linear-gradient(90deg, hsla(${_h},48%,50%,.28), hsla(${_h},55%,62%,.92))`;
    return `<div class="cat-bar"${rowAttrs}><div class="cbi gem" style="background:${getCatColor(c.cat)};--gem-edge:${getCatEdge(c.cat)}">${getCatIcon(c.cat, isIncome)}</div><div class="cbi-info"><div class="cbi-top"><div class="cbi-name" style="display:flex;align-items:center">${esc(c.cat)}${chev}</div><div class="cbi-pct">${pct}%</div></div><div class="bar-track"><div class="bar-fill${isIncome?' bar-fill-inc':''}" style="${_grad}"></div></div></div><div class="cbi-amt">${fmtMoney(c.amt)}</div></div>` + (open ? subCatDrillHTML(c.cat, c.amt) : '');
  }).join('');
...
function toggleAnalyticsAdvanced(){
  const tabs = document.getElementById('an-tabs');
  const btn = document.getElementById('an-more-btn');
  if (!tabs) return;
  const showing = tabs.style.display !== 'none';
  if (showing){
    // collapse back to Overview only
    tabs.style.display = 'none';
    if (btn) btn.classList.remove('active');
    try { setAnalyticsTab('overview', document.querySelector('.an-tab')); } catch(e){}
  } else {
...
function renderSettings() {
  ...
  if (tiSub) tiSub.textContent = (S.trackIncome ? 'On' : 'Off') + (S.family ? ' · just for you' : '');
  try { _aiSub(); } catch(e) {}
  ...
  if (proSub) proSub.textContent = isPro() ? (planLabel() + ' \u2713 \u2014 thank you!') : 'Upgrade for family sharing and more';
...
// ═══════════════════════════════════════════
function goTab(tab) {
  try { closeAllSettingsDetails(); } catch(e) {} // never leave a Settings sub-page open across tabs
  ...
  try { playEntrance('page-' + tab); } catch(e) {} // gentle stagger on every tab open
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Match a card/account named in free text against the user's real accounts.
// Understands account-name words, account types (credit/debit/savings/cash/
// everyday), and common bank shorthands (cba, commbank, amex, anz, nab…).
const _BANK_WORDS = {
  cba:'commonwealth', commbank:'commonwealth', comm:'commonwealth', commonwealth:'commonwealth',
  wbc:'westpac', westpac:'westpac', nab:'nab', anz:'anz', ing:'ing', amex:'amex',
  macq:'macquarie', macquarie:'macquarie', bankwest:'bankwest', suncorp:'suncorp',
  ubank:'ubank', bendigo:'bendigo', citi:'citi', hsbc:'hsbc', visa:'visa', mastercard:'mastercard'
};
function _scribeAccount(text){
  if (!S.accts || !S.accts.length) return null;
  const n = ' ' + String(text||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim() + ' ';
  const words = n.trim().split(' ');
  let best = null;
  // 1) direct account-name word match (strongest)
  S.accts.forEach(a => {
    const toks = String(a.name||'').toLowerCase().split(/[^a-z0-9]+/).filter(w => w.length >= 3);
    toks.forEach(w => { if (n.indexOf(' '+w+' ') >= 0 && !best) best = { account:a.name, hit:w }; });
  });
  // 2) bank shorthand → an account whose name contains that bank
  if (!best) {
    for (const w of words) {
      const full = _BANK_WORDS[w];
      if (!full) continue;
      const a = S.accts.find(x => { const toks = String(x.name||'').toLowerCase().split(/[^a-z0-9]+/); return toks.includes(full) || toks.includes(w); });
      if (a) { best = { account:a.name, hit:w }; break; }
    }
  }
  // 3) account-type word (credit / debit / savings / cash / everyday / checking)
  if (!best) {
    const typeHit = { credit:'Credit', debit:'Credit', savings:'Savings', saving:'Savings', cash:'Cash', everyday:'Checking', checking:'Checking', transaction:'Checking' };
    for (const w of words) {
      const ty = typeHit[w];
      if (!ty) continue;
      const a = S.accts.find(x => String(x.type||'').toLowerCase() === ty.toLowerCase() || String(x.name||'').toLowerCase().includes(w));
      if (a) { best = { account:a.name, hit:w }; break; }
    }
  }
  return best;
...
// Spelled-out amounts → number: "fifty six" → 56, "two hundred" → 200, "one thousand" → 1000.
function _wordsToAmount(text){
  const U = { zero:0,one:1,two:2,three:3,four:4,five:5,six:6,seven:7,eight:8,nine:9,ten:10,
    eleven:11,twelve:12,thirteen:13,fourteen:14,fifteen:15,sixteen:16,seventeen:17,eighteen:18,nineteen:19,
    twenty:20,thirty:30,forty:40,fifty:50,sixty:60,seventy:70,eighty:80,ninety:90 };
  const toks = String(text||'').toLowerCase().match(/[a-z]+/g) || [];
  let total = 0, cur = 0, saw = false;
  for (const w of toks){
    if (U[w] != null){ cur += U[w]; saw = true; }
    else if (w === 'hundred'){ cur = (cur || 1) * 100; saw = true; }
    else if (w === 'thousand'){ total += (cur || 1) * 1000; cur = 0; saw = true; }
    else if (w === 'and' && saw){ /* keep going */ }
    else if (saw){ break; } // stop at the first non-number word after we've started
  }
  const n = total + cur;
  return saw && n > 0 ? n : null;
}
function _quickParseLine(line) {
  ...
  // Recognise the brand first, then remove it before reading amount/account —
  // stops brands that contain numbers ("7 eleven", "google one") or account words
  // ("cash app") from hijacking those slots.
  const _bkw0 = (merchantFor(t) || {}).keyword || '';
  const _stripB = (str) => {
    if (!_bkw0) return str;
    try { const pat = _bkw0.split(/[^a-z0-9]+/).filter(Boolean).map(w => w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).join('[^a-z0-9]+'); return str.replace(new RegExp('\\b'+pat+'\\b','gi'), ' '); }
    catch(e) { return str; }
  };
  const _amtText = _stripB(t);
  // amount: first money-looking number; supports $8.75, 1,200, 1.2k
  const m = _amtText.match(/(?:\$|£|€)?\s*(\d{1,3}(?:,\d{3})+|\d+(?:\.\d+)?)(?!\s*days?\s+ago)(k)?\b/i);
  let amount = null;
  if (m) { amount = parseFloat(m[1].replace(/,/g, '')); if (m[2]) amount *= 1000; }
  else { amount = _wordsToAmount(_amtText); }   // spelled-out numbers: "fifty six", "two hundred"
  if (!(amount > 0)) return null;
  // date: "yesterday" or a trailing ISO date, else today
  let date = todayISO();
  if (/\byesterday\b/i.test(t)) date = isoLocal(new Date(Date.now() - 86400000));
  const agoM = t.match(/\b(\d{1,2})\s*days?\s*ago\b/i);
  if (agoM) date = isoLocal(new Date(Date.now() - (parseInt(agoM[1],10)||0)*86400000));
  const wdM = t.match(/\blast\s+(sun|mon|tue|wed|thu|fri|sat)[a-z]*\b/i);
  if (wdM) { const map={sun:0,mon:1,tue:2,wed:3,thu:4,fri:5,sat:6}; const want=map[wdM[1].toLowerCase().slice(0,3)]; const d=new Date(); let diff=(d.getDay()-want+7)%7; if(diff===0)diff=7; d.setDate(d.getDate()-diff); date=isoLocal(d); }
  const dm = t.match(/\b(\d{4}-\d{2}-\d{2})\b/);
  if (dm) date = dm[1];
  // account / card named in the line (e.g. "with cba credit card")
  const acctMatch = _scribeAccount(_amtText);
  const account = acctMatch ? acctMatch.account : null;
  // Merchant, order-independent: (1) a known brand anywhere in the line wins,
  // (2) else an explicit "at/from X" phrase, (3) else the leftover words.
  let merchantPhrase = '';
  let brandKw = '';
  try {
    const bm = merchantFor(t);
    if (bm && bm.keyword) { brandKw = bm.keyword; merchantPhrase = bm.keyword.replace(/\b[a-z]/g, ch => ch.toUpperCase()); }
  } catch(e) {}
  if (!merchantPhrase) {
    const mm = t.match(/\b(?:at|from|@)\s+([a-z0-9'&.\- ]+?)(?=\s+(?:with|on|using|via|paid|for|yesterday|today|last|next)\b|\s*$)/i);
    if (mm) merchantPhrase = mm[1].trim();
  }
  // description: strip amount, the account phrase, prepositions, filler
  let desc = t.replace(m ? m[0] : /\b(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred|thousand)\b/gi, ' ');
  // remove "with/on/using/paid with … <account/card/bank/type>" chunk
  desc = desc.replace(/\b(?:with|on|using|via|from|paid\s+with|paid\s+by)\s+([a-z0-9]+\s+){0,3}(card|credit|debit|account|acct|cba|commbank|amex|visa|mastercard|anz|nab|westpac|ing|savings?|everyday|checking|cash)\b/gi, ' ');
  if (acctMatch && acctMatch.hit) desc = desc.replace(new RegExp('\\b'+acctMatch.hit.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','gi'), ' ');
  if (brandKw) desc = desc.replace(new RegExp('\\b'+brandKw.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','gi'), ' ');
  desc = desc
    .replace(/\b(received|got paid|got|spent|paid me|paid|bought|for|on|at|from|via|using|with|to|my|salary income|yesterday|today)\b/gi, ' ')
    .replace(/\b\d{4}-\d{2}-\d{2}\b/, ' ')
    .replace(/\b(credit|debit|card|account|acct)\b/gi, ' ')
    .replace(/[+$£€]/g, ' ')
    .replace(/\s+/g, ' ').trim();
  if (merchantPhrase) desc = merchantPhrase;
  ...
  return { amount, type: isInc ? 'income' : 'expense', date, description: desc, category, catSource, account: account, accountConfident: !!account };
}
...
let _voiceRec = null;
function toggleVoiceAdd() { openVoiceMode(); }

// ── VOICE MODE · full-screen live mic waveform + speech → confirm popup ───────
let _vaCtx = null, _vaAnalyser = null, _vaStream = null, _vaRAF = 0, _vaRec = null, _vaHeard = '';
...
async function openVoiceMode() {
  try { closeOverlay('quick-overlay'); } catch(e){}
  const ov = document.getElementById('voice-overlay'); if (!ov) return;
  _vaHeard = '';
  const t = document.getElementById('voice-transcript');
  if (t) { t.textContent = 'Say it like \u201c8.75 coffee at Woolworths with CBA\u201d'; t.classList.add('hint'); }
  const title = document.getElementById('voice-title'); if (title) title.textContent = 'Listening\u2026';
  ov.classList.add('open');
  try { await _vaStartAudio(); } catch(e) { /* mic denied → keep UI, no bars */ }
  _vaStartSpeech();
}
async function _vaStartAudio() {
  const AC = window.AudioContext || window.webkitAudioContext;
  if (!AC || !navigator.mediaDevices) throw new Error('no audio');
  _vaStream = await navigator.mediaDevices.getUserMedia({ audio: true });
  _vaCtx = new AC();
  const src = _vaCtx.createMediaStreamSource(_vaStream);
  _vaAnalyser = _vaCtx.createAnalyser();
  _vaAnalyser.fftSize = 256; _vaAnalyser.smoothingTimeConstant = 0.72;
  src.connect(_vaAnalyser);
  const c = document.getElementById('voice-canvas'); if (!c) return;
  const ctx = c.getContext('2d');
  const bins = _vaAnalyser.frequencyBinCount;
  const data = new Uint8Array(bins);
  const BARS = 44;
  function frame() {
    if (!_vaAnalyser) return;
    const dpr = window.devicePixelRatio || 1;
    const W = c.clientWidth, H = c.clientHeight;
    if (c.width !== W*dpr || c.height !== H*dpr) { c.width = W*dpr; c.height = H*dpr; }
    ctx.setTransform(dpr,0,0,dpr,0,0);
    ctx.clearRect(0,0,W,H);
    _vaAnalyser.getByteFrequencyData(data);
    const bw = W / BARS, step = Math.max(1, Math.floor(bins*0.7 / BARS));
    for (let i=0;i<BARS;i++) {
      const v = (data[i*step] || 0) / 255;
      const bh = Math.max(4, Math.pow(v, 0.8) * H * 0.92);
      const x = i*bw + bw*0.22, w = bw*0.56, y = (H - bh)/2;
      const g = ctx.createLinearGradient(0, y, 0, y+bh);
      g.addColorStop(0, '#FF6B6B'); g.addColorStop(0.5, '#F03A3A'); g.addColorStop(1, '#B81616');
      ctx.fillStyle = g;
      _roundRectPath(ctx, x, y, w, bh, w/2); ctx.fill();
    }
    _vaRAF = requestAnimationFrame(frame);
  }
  frame();
}
function _vaStartSpeech() {
  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) { const t=document.getElementById('voice-transcript'); if(t){t.classList.remove('hint');t.textContent='Voice isn\u2019t supported in this browser \u2014 type instead.';} return; }
  try {
    const rec = new SR();
    ...
    rec.onresult = (ev) => {
      ...
      txt = txt.trim();
      if (txt) { _vaHeard = txt; const t=document.getElementById('voice-transcript'); if(t){ t.classList.remove('hint'); t.textContent = txt; } }
    };
    rec.onerror = () => {};
    rec.onend = () => { _vaRec = null; };
    _vaRec = rec; rec.start();
  } catch(e) {}
}
function _vaStop() {
  try { if (_vaRAF) cancelAnimationFrame(_vaRAF); } catch(e){} _vaRAF = 0;
  try { if (_vaRec) _vaRec.stop(); } catch(e){} _vaRec = null;
  try { if (_vaStream) _vaStream.getTracks().forEach(tr => tr.stop()); } catch(e){} _vaStream = null;
  try { if (_vaCtx) _vaCtx.close(); } catch(e){} _vaCtx = null; _vaAnalyser = null;
}
function closeVoiceOverlay() {
  _vaStop();
  const ov = document.getElementById('voice-overlay'); if (ov) ov.classList.remove('open');
}
function voiceFinish() {
  const heard = (_vaHeard || '').trim();
  closeVoiceOverlay();
  if (!heard) return;
  const parsed = _quickParseLine(heard);
  if (!parsed) { toast('Didn\u2019t catch an amount \u2014 try again'); return; }
  openScribeConfirm(parsed);   // straight into the clean confirm popup
}
function openQuickAdd() {
  ...
  const hl = document.getElementById('quick-hl'); if (hl) hl.innerHTML = '';
  const rb = document.getElementById('receipt-btn'); if (rb) rb.style.display = betaOn('receipt') ? 'flex' : 'none';
...
// ── SCRIBE premium typing · live token highlight behind the text (caret-aligned) ──
function _scribeHLScroll(){ const ta=document.getElementById('quick-input'), hl=document.getElementById('quick-hl'); if(ta&&hl){ hl.scrollTop=ta.scrollTop; hl.scrollLeft=ta.scrollLeft; } }
function _scribeHLBuild(text){
  const ranges=[];
  const push=(a,b,cls)=>{ if(b>a) ranges.push([a,b,cls]); };
  let re, m;
  // numbers (optional currency prefix, optional k) — the star of the show
  re=/(?:\$|£|€)?\d[\d,]*(?:\.\d+)?k?/gi;
  while((m=re.exec(text))){ if(/\d/.test(m[0])) push(m.index, m.index+m[0].length, 'hl-num'); }
  // dates
  re=/\b(yesterday|today|\d+\s*days?\s*ago|last\s+(?:sun|mon|tue|wed|thu|fri|sat)[a-z]*|\d{4}-\d{2}-\d{2})\b/gi;
  while((m=re.exec(text))){ push(m.index, m.index+m[0].length, 'hl-date'); }
  // merchant + account (highlight the recognised keyword wherever it sits)
  try {
    const bm = merchantFor(text);
    if (bm && bm.keyword){ const km = new RegExp('\\b'+bm.keyword.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','i'); const mm=km.exec(text); if(mm) push(mm.index, mm.index+mm[0].length, 'hl-mer'); }
  } catch(e){}
  try {
    const am = _scribeAccount(text);
    if (am && am.hit){ const km = new RegExp('\\b'+am.hit.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','i'); const mm=km.exec(text); if(mm) push(mm.index, mm.index+mm[0].length, 'hl-acct'); }
  } catch(e){}
  ranges.sort((a,b)=>a[0]-b[0]);
  let out='', pos=0;
  for (const [a,b,cls] of ranges){ if (a<pos) continue; out += esc(text.slice(pos,a)); out += '<span class="'+cls+'">'+esc(text.slice(a,b))+'</span>'; pos=b; }
  out += esc(text.slice(pos));
  return out;
}
function _scribeHL(){
  const ta=document.getElementById('quick-input'), hl=document.getElementById('quick-hl');
  if(!ta||!hl) return;
  hl.innerHTML = _scribeHLBuild(ta.value) + '\n';
  _scribeHLScroll();
}
...
async function quickAddCommit() {
  ...
  // Single entry → open the clean confirm popup so the user can verify/fix fields
  // on the go (amount, merchant, category, account, date) instead of a raw box.
  if (parsed.length === 1) { try { closeOverlay('quick-overlay'); } catch(e){} openScribeConfirm(parsed[0]); return; }
...
function openAddModal(forceForm) {
  ...
  // Scribe is a BETA feature, so the beta flag is the single source of truth.
  // Reading it this way means a stale prizm_add_mode from earlier testing can
  // never resurrect Scribe for someone who has Beta switched off.
  if (!forceForm && betaOn('scribe')) { openQuickAdd(); return; }
  S.editingTxId = null;
...
async function submitTx() {
  ...
  if (editingId) {
    ...
    await apiPost({ action: 'addTransaction', id: 'tx_' + Date.now() + '_' + Math.random().toString(36).slice(2,6), ts: Date.now(), type: S.type, amount: amt, category: packCat(S.cat, S.subcat), description: desc, account: acct, date, notes, memberId: memVal });
    celebrateAdd();
...
async function submitAcct() {
  ...
  if (editingId) {
    let store = bal;
    if (S.mode === 'sheets') { const ex = S.accts.find(a => a.id === editingId); store = Math.round((bal - _accountTxSum(ex ? ex.name : name)) * 100) / 100; }
    await apiPost({ action: 'updateAccount', id: editingId, name, type, balance: store });
    toast('✓ Account updated');
  } else {
    await apiPost({ action: 'addAccount', id: 'ac_' + Date.now() + '_' + Math.random().toString(36).slice(2,5), name, type, balance: bal });
    toast('✓ Account added');
  }
}
...
async function submitRecurring() {
  ...
  if (editingId) {
    ...
    await apiPost({ action: 'addRecurring', id: 'rec_' + Date.now() + '_' + Math.random().toString(36).slice(2,5), name, type: S.recType, amount: amt, category: S.recCat, account: acct, frequency: freq, nextDate: next, memberId: S.member ? S.member.id : null, active: true });
    toast('✓ Added');
  }
  checkRecurringDue();
}
...
async function submitBudget() {
  ...
  if (!budgetId) { const existing = (S.budgets || []).find(b => b.category === cat); budgetId = existing ? existing.id : 'bg_' + Date.now() + '_' + Math.random().toString(36).slice(2,5); }
...
function openMemberModal() {
  ...
  if ((S.members || []).length >= memberLimit()) { toast('The ' + planLabel() + ' plan supports up to ' + memberLimit() + ' members'); openProModal(); return; }
  S.newMemberPin = '';
...
// ═══════════════════════════════════════════════════════════════════════════
//  ONE-TAP GOOGLE ENGINE · direct Sheets API over the drive.file scope
//  No Apps Script, no "enable the API" switch, no public /exec endpoint.
//  The app may only see files it created itself (drive.file = non-sensitive),
//  so there is nothing to password-protect and nothing for outsiders to probe.
//  Old Apps Script ('sheets' + S.url=https://script...) setups keep working
//  untouched; this engine activates only when prizm_engine === 'gapi'.
// ═══════════════════════════════════════════════════════════════════════════
var GS_SCOPE = 'https://www.googleapis.com/auth/drive.file';
// Needed ONLY for "join a family" (Google Picker lets an invited member grant
// this app access to the shared ledger). Create a browser API key with the
// Picker API enabled in your Google Cloud console and paste it here.
var PICKER_API_KEY = 'AIzaSyBX239KiztGSHUQOXnD2Ey4Ohoqp4SONfI';

var GS_TABS = {
  Transactions: ['ID','Date','Description','Amount','Type','Category','Account','Notes','MemberID','TS'],
  Accounts:     ['ID','Name','Type','Balance'],
  Members:      ['ID','Name','PIN','Color'],
  Recurring:    ['ID','Name','Amount','Type','Category','Account','Frequency','NextDate','MemberID','Active'],
  Budgets:      ['ID','Category','Limit'],
  Settings:     ['Key','Value']
};
var GS_COLS = { Transactions:'J', Accounts:'D', Members:'D', Recurring:'J', Budgets:'C', Settings:'B' };

function _gsMode(){ try { return localStorage.getItem('prizm_engine') === 'gapi'; } catch(e){ return false; } }
function _gsSid(){ try { return localStorage.getItem('prizm_gs_id') || ''; } catch(e){ return ''; } }

var _gsTok = null, _gsTokExp = 0, _gsMetaCache = null;
function gsTokenSilent(){ return _gsTokenGet(''); }
function gsTokenInteractive(){ return _gsTokenGet('consent-if-needed'); }
function _gsTokenGet(mode){
  if (_gsTok && Date.now() < _gsTokExp - 60000) return Promise.resolve(_gsTok);
  return new Promise(function(resolve, reject){
    if (!gisReady()) return reject(new Error('google_not_loaded'));
    try {
      var tc = google.accounts.oauth2.initTokenClient({
        client_id: GOOGLE_CLIENT_ID,
        scope: GS_SCOPE,
        callback: function(resp){
          if (resp && resp.access_token) {
            _gsTok = resp.access_token;
            _gsTokExp = Date.now() + (Number(resp.expires_in || 3500) * 1000);
            resolve(_gsTok);
          } else reject(new Error((resp && resp.error) || 'no_token'));
        },
        error_callback: function(err){ reject(new Error((err && (err.type || err.message)) || 'popup_closed')); }
      });
      tc.requestAccessToken(mode === '' ? { prompt: '' } : {});
    } catch(e){ reject(e); }
  });
}

async function gsFetch(method, url, body, retried){
  var tok;
  try { tok = await gsTokenSilent(); }
  catch(e){ tok = await gsTokenInteractive(); }
  var r = await fetch(url, {
    method: method,
    headers: { 'Authorization': 'Bearer ' + tok, 'Content-Type': 'application/json' },
    body: body ? JSON.stringify(body) : undefined
  });
  if (r.status === 401 && !retried) { _gsTok = null; return gsFetch(method, url, body, true); }
  var d = null;
  try { d = await r.json(); } catch(e){}
  if (!r.ok) {
    var msg = (d && d.error && d.error.message) || ('HTTP ' + r.status);
    if (r.status >= 500 || r.status === 429) { var err = new Error(msg); err.transient = true; throw err; }
    return { error: msg };
  }
  return d || {};
}

function _gsBase(){ return 'https://sheets.googleapis.com/v4/spreadsheets/' + encodeURIComponent(_gsSid()); }

async function gsMeta(){
  if (_gsMetaCache) return _gsMetaCache;
  var d = await gsFetch('GET', _gsBase() + '?fields=sheets.properties');
  if (d.error) throw new Error(d.error);
  var map = {};
  (d.sheets || []).forEach(function(sh){ map[sh.properties.title] = sh.properties.sheetId; });
  _gsMetaCache = map;
  return map;
}
...
async function gsLoadAll(){
  var ranges = Object.keys(GS_TABS).map(function(t){ return 'ranges=' + encodeURIComponent("'" + t + "'!A2:" + GS_COLS[t]); }).join('&');
  var d = await gsFetch('GET', _gsBase() + '/values:batchGet?valueRenderOption=UNFORMATTED_VALUE&' + ranges);
  if (d.error) throw new Error(d.error);
  var vr = {}; (d.valueRanges || []).forEach(function(x){
    var t = String(x.range || '').split('!')[0].replace(/^'+|'+$/g, '');
    vr[t] = x.values || [];
  });
  var has = function(r){ return r && r[0] !== '' && r[0] != null; };
  var settings = {};
  (vr.Settings || []).forEach(function(r){ if (r[0] && r[0] !== 'app_secret') settings[r[0]] = r[1]; });
  return {
    transactions: (vr.Transactions || []).filter(has).map(function(r){ return { id:String(r[0]), date:String(r[1]||''), description:r[2]||'', amount:Number(r[3])||0, type:r[4]||'expense', category:r[5]||'Other', account:r[6]||'', notes:r[7]||'', memberId:r[8]||'', ts:r[9]||'' }; }),
    accounts:   (vr.Accounts   || []).filter(has).map(function(r){ return { id:String(r[0]), name:r[1]||'', type:r[2]||'', balance:Number(r[3])||0 }; }),
    members:    (vr.Members    || []).filter(has).map(function(r){ return { id:String(r[0]), name:r[1]||'', pin:String(r[2]==null?'':r[2]), color:r[3]||'#C6A15B' }; }),
    recurring:  (vr.Recurring  || []).filter(has).map(function(r){ return { id:String(r[0]), name:r[1]||'', amount:Number(r[2])||0, type:r[3]||'expense', category:r[4]||'Other', account:r[5]||'', frequency:r[6]||'Monthly', nextDate:String(r[7]||''), memberId:r[8]||'', active:r[9] }; }),
    budgets:    (vr.Budgets    || []).filter(has).map(function(r){ return { id:String(r[0]), category:r[1]||'', limit:Number(r[2])||0 }; }),
    settings: settings
  };
}

// ── Row helpers ──
async function _gsColA(tab){
  var d = await gsFetch('GET', _gsBase() + '/values/' + encodeURIComponent("'" + tab + "'!A:A"));
  if (d.error) throw new Error(d.error);
  return (d.values || []).map(function(r){ return String(r[0] == null ? '' : r[0]); });
}
async function _gsFindRow(tab, id){
  var col = await _gsColA(tab);
  for (var i = 1; i < col.length; i++) if (col[i] === String(id)) return i + 1;
  return -1;
}
async function _gsAppend(tab, rowVals){
  var d = await gsFetch('POST', _gsBase() + '/values/' + encodeURIComponent("'" + tab + "'!A1") + ':append?valueInputOption=RAW&insertDataOption=INSERT_ROWS', { values: [rowVals] });
  if (d.error) throw new Error(d.error);
  return { success: true };
}
async function _gsUpdateRow(tab, row, rowVals){
  var range = "'" + tab + "'!A" + row + ':' + GS_COLS[tab] + row;
  var d = await gsFetch('PUT', _gsBase() + '/values/' + encodeURIComponent(range) + '?valueInputOption=RAW', { values: [rowVals] });
  if (d.error) throw new Error(d.error);
  return { success: true };
}
async function _gsDeleteRow(tab, row){
  var meta = await gsMeta();
  if (meta[tab] == null) return { success: true };
  var d = await gsFetch('POST', _gsBase() + ':batchUpdate', {
    requests: [{ deleteDimension: { range: { sheetId: meta[tab], dimension: 'ROWS', startIndex: row - 1, endIndex: row } } }]
  });
  if (d.error) throw new Error(d.error);
  return { success: true };
}
async function _gsUpsert(tab, id, rowVals){
  var row = await _gsFindRow(tab, id);
  if (row === -1) return _gsAppend(tab, rowVals);
  return _gsUpdateRow(tab, row, rowVals);
}

// ── Write actions · mirrors the old Apps Script ACTIONS map, client-side ──
async function gsApplyAction(b){
  switch (b.action) {
    case 'addTransaction': {
      var ex = await _gsFindRow('Transactions', b.id);
      if (ex !== -1) return { success: true, dup: true };
      return _gsAppend('Transactions', [b.id, b.date, b.description, Number(b.amount), b.type, b.category, b.account || '', b.notes || '', b.memberId || '', b.ts || Date.now()]);
    }
    case 'updateTransaction':
      return _gsUpsert('Transactions', b.id, [b.id, b.date, b.description, Number(b.amount), b.type, b.category, b.account || '', b.notes || '', b.memberId || '', b.ts || Date.now()]);
    case 'deleteTx': {
      var r1 = await _gsFindRow('Transactions', b.id);
      return r1 === -1 ? { success: true } : _gsDeleteRow('Transactions', r1);
    }
    case 'addAccount':
      return _gsAppend('Accounts', [b.id, b.name, b.type, Number(b.balance) || 0]);
    case 'updateAccount':
      return _gsUpsert('Accounts', b.id, [b.id, b.name, b.type, Number(b.balance) || 0]);
    case 'deleteAccount': {
      var r2 = await _gsFindRow('Accounts', b.id);
      return r2 === -1 ? { success: true } : _gsDeleteRow('Accounts', r2);
    }
    case 'addMember':
      return _gsAppend('Members', [b.id, b.name, b.pin, b.color]);
    case 'updateMember':
      return _gsUpsert('Members', b.id, [b.id, b.name, b.pin, b.color]);
    case 'deleteMember': {
      var r3 = await _gsFindRow('Members', b.id);
      return r3 === -1 ? { success: true } : _gsDeleteRow('Members', r3);
    }
    case 'addRecurring':
      return _gsAppend('Recurring', [b.id, b.name, Number(b.amount), b.type, b.category, b.account || '', b.frequency, b.nextDate, b.memberId || '', b.active !== false]);
    case 'updateRecurring':
      return _gsUpsert('Recurring', b.id, [b.id, b.name, Number(b.amount), b.type, b.category, b.account || '', b.frequency, b.nextDate, b.memberId || '', b.active !== false]);
    case 'deleteRecurring': {
      var r4 = await _gsFindRow('Recurring', b.id);
      return r4 === -1 ? { success: true } : _gsDeleteRow('Recurring', r4);
    }
    case 'setBudget':
      return _gsUpsert('Budgets', b.id, [b.id, b.category, Number(b.limit) || 0]);
    case 'deleteBudget': {
      var r5 = await _gsFindRow('Budgets', b.id);
      return r5 === -1 ? { success: true } : _gsDeleteRow('Budgets', r5);
    }
    case 'updateSettings': {
      var col = await _gsColA('Settings');
      var row = -1;
      for (var i = 1; i < col.length; i++) if (col[i] === String(b.key)) { row = i + 1; break; }
      if (row === -1) return _gsAppend('Settings', [b.key, b.value]);
      return _gsUpdateRow('Settings', row, [b.key, b.value]);
    }
    // Not applicable without an Apps Script backend — succeed quietly.
    case 'setSecret':
    case 'enableMonthlyReport':
    case 'disableMonthlyReport':
      return { success: true, noop: true };
    default:
      console.warn('gsApplyAction: unknown action', b.action);
      return { success: true, noop: true };
  }
}
...
async function gsCreateSpreadsheet(title){
  var body = {
    ...
    sheets: Object.keys(GS_TABS).map(function(t){ return { properties: { title: t, gridProperties: { frozenRowCount: 1 } } }; })
  };
  var d = await gsFetch('POST', 'https://sheets.googleapis.com/v4/spreadsheets', body);
  if (d.error) throw new Error(d.error);
  var sid = d.spreadsheetId;
  var data = Object.keys(GS_TABS).map(function(t){ return { range: "'" + t + "'!A1", values: [GS_TABS[t]] }; });
  var d2 = await gsFetch('POST', 'https://sheets.googleapis.com/v4/spreadsheets/' + encodeURIComponent(sid) + '/values:batchUpdate', { valueInputOption: 'RAW', data: data });
  if (d2.error) throw new Error(d2.error);
  return sid;
}

// ── One-tap setup (solo + family create) ──
async function gsOneTapSetup(family){
  if (!gisReady()) { toast('Google sign-in is still loading — try again in a moment.'); return; }
  try {
    loader(true);
    await gsTokenInteractive();
    var famName = '';
    if (family) {
      var fn = document.getElementById('family-name');
      famName = ((fn && fn.value) || '').trim() || 'My Family';
    }
    var title = family ? ('Prizm Ledger · ' + famName) : 'Prizm Ledger';
    var sid = await gsCreateSpreadsheet(title);
    localStorage.setItem('prizm_engine', 'gapi');
    localStorage.setItem('prizm_gs_id', sid);
    localStorage.setItem('prizm_url', 'gapi:' + sid);
    localStorage.setItem('prizm_mode', 'sheets');
    localStorage.setItem('prizm_family', family ? 'true' : 'false');
    S.mode = 'sheets'; S.family = !!family; S.url = 'gapi:' + sid;
    _gsMetaCache = null;
    if (family) {
      S.familyName = famName;
      localStorage.setItem('prizm_family_name', famName);
      apiPost({ action: 'updateSettings', key: 'familyName', value: famName });
      apiPost({ action: 'updateSettings', key: 'engine', value: 'gapi-v1' });
      loader(false);
      toast('✅ Family ledger created in your Drive');
      showSetup('sp-gs-invite');
    } else {
      apiPost({ action: 'updateSettings', key: 'engine', value: 'gapi-v1' });
      startPolling();
      loader(false);
      toast('✅ Private Sheet created in your Drive');
      S.onboardingNext = 'enterApp';
      showSetup('sp-prefs');
    }
  } catch(e){
    loader(false);
    if (!_gisCancelled(e)) { console.warn('gsOneTapSetup:', e); toast('⚠️ Could not finish Google setup — ' + (e.message || 'try again')); }
  }
}

// ── Invite family members (Drive share — the normal Docs-style email) ──
async function gsInviteMembers(){
  var inp = document.getElementById('gs-invite-emails');
  var raw = ((inp && inp.value) || '').trim();
  if (!raw) { toast('Enter at least one email address'); return; }
  var emails = raw.split(/[\s,;]+/).filter(function(x){ return /.+@.+\..+/.test(x); });
  if (!emails.length) { toast('⚠️ Those don’t look like email addresses'); return; }
  loader(true);
  var ok = 0, fail = 0;
  for (var i = 0; i < emails.length; i++) {
    try {
      var d = await gsFetch('POST', 'https://www.googleapis.com/drive/v3/files/' + encodeURIComponent(_gsSid()) + '/permissions?sendNotificationEmail=true', { role: 'writer', type: 'user', emailAddress: emails[i] });
      if (d.error) fail++; else ok++;
    } catch(e){ fail++; }
  }
  loader(false);
  if (ok) toast('✅ Invited ' + ok + (ok > 1 ? ' people' : ' person') + (fail ? (' · ' + fail + ' failed') : ''));
  else toast('⚠️ Could not send invites — check the addresses and try again');
  if (ok && inp) inp.value = '';
}
function gsInviteDone(){
  S.onboardingNext = 'profilePicker';
  showProfilePicker();
}

// ── Join a family (invited member picks the shared ledger via Google Picker) ──
function _gsLoadPicker(){
  return new Promise(function(resolve, reject){
    if (window.google && google.picker) return resolve();
    var sc = document.createElement('script');
    sc.src = 'https://apis.google.com/js/api.js';
    sc.onload = function(){ gapi.load('picker', { callback: resolve, onerror: reject }); };
    sc.onerror = reject;
    document.head.appendChild(sc);
  });
}
async function gsJoinFamily(){
  if (!gisReady()) { toast('Google sign-in is still loading — try again in a moment.'); return; }
  if (!PICKER_API_KEY) {
    showCustomAlert('Almost ready', 'Joining a family needs the app’s Google Picker key, which hasn’t been configured in this build yet. Ask the family admin to check the Prizm setup guide.');
    return;
  }
  try {
    var tok = await gsTokenInteractive();
    await _gsLoadPicker();
    var view = new google.picker.DocsView(google.picker.ViewId.SPREADSHEETS).setIncludeFolders(false);
    var picker = new google.picker.PickerBuilder()
      .addView(view)
      .setOAuthToken(tok)
      .setDeveloperKey(PICKER_API_KEY)
      .setTitle('Pick your family’s Prizm ledger')
      .setCallback(function(data){
        if (data.action !== google.picker.Action.PICKED) return;
        var sid = data.docs && data.docs[0] && data.docs[0].id;
        if (sid) _gsFinishJoin(sid);
      })
      .build();
    picker.setVisible(true);
  } catch(e){
    if (!_gisCancelled(e)) { console.warn('gsJoinFamily:', e); toast('⚠️ Could not open the Google picker'); }
  }
}
async function _gsFinishJoin(sid){
  loader(true);
  try {
    localStorage.setItem('prizm_engine', 'gapi');
    localStorage.setItem('prizm_gs_id', sid);
    _gsMetaCache = null;
    var d = await gsLoadAll();
    localStorage.setItem('prizm_url', 'gapi:' + sid);
    localStorage.setItem('prizm_mode', 'sheets');
    localStorage.setItem('prizm_family', 'true');
    S.mode = 'sheets'; S.family = true; S.url = 'gapi:' + sid;
    S.familyName = (d.settings && d.settings.familyName) || 'Family';
    localStorage.setItem('prizm_family_name', S.familyName);
    applyRemoteData(d);
    startPolling();
    loader(false);
    toast('✅ Joined ' + S.familyName);
    S.onboardingNext = 'profilePicker';
    showProfilePicker();
  } catch(e){
    loader(false);
    localStorage.removeItem('prizm_engine');
    localStorage.removeItem('prizm_gs_id');
    console.warn('_gsFinishJoin:', e);
    toast('⚠️ That doesn’t look like a Prizm ledger — pick the sheet your family shared with you');
  }
}

// ═══════════════════════════════════════════════════════════════════════════
//  PRIZM WRAPPED · a celebratory month / financial-year recap, once each period,
//  shareable to WhatsApp. Built entirely from local data.
// ═══════════════════════════════════════════════════════════════════════════
function _dayName(ms){ return ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][new Date(ms).getDay()]; }
function buildWrapped(period){
  const now = new Date();
  let start, end, label, sub;
  if (period === 'fy'){
    // Australian FY: 1 Jul → 30 Jun. After 1 Jul we recap the FY that just ended.
    const endYear = now.getFullYear();
    start = new Date(endYear-1, 6, 1).getTime();
    end   = new Date(endYear, 6, 1).getTime();
    label = 'FY' + String(endYear-1).slice(2) + '–' + String(endYear).slice(2);
    sub = 'Your financial year, wrapped';
  } else {
    const pm = new Date(now.getFullYear(), now.getMonth()-1, 1);
    start = pm.getTime();
    end = new Date(now.getFullYear(), now.getMonth(), 1).getTime();
    label = pm.toLocaleString('default', { month: 'long', year: 'numeric' });
    sub = 'Your month, wrapped';
  }
  let inc=0, exp=0, txCount=0, biggest=null;
  const days=new Set(), catExp={}, merch={}, dow=[0,0,0,0,0,0,0];
  (S.txs||[]).forEach(t=>{
    const ms=dateMs(t.date); if (ms<start||ms>=end) return;
    txCount++; const a=parseFloat(t.amount)||0;
    const k=_emoDayKey(ms); if(k) days.add(k);
    if (t.type==='income'){ inc+=a; return; }
    exp+=a; dow[new Date(ms).getDay()]+=a;
    const c=baseCat(t.category); catExp[c]=(catExp[c]||0)+a;
    const d=(t.description||c).trim(); if(d) merch[d]=(merch[d]||0)+a;
    if(!biggest||a>biggest.amt) biggest={desc:d,amt:a,cat:c};
  });
  if (!txCount) return null;
  const top=(o)=>{let k=null,v=-1;for(const x in o)if(o[x]>v){v=o[x];k=x;}return k?{name:k,amt:v}:null;};
  const topCat=top(catExp), topMerch=top(merch);
  let busyIdx=0; for(let i=1;i<7;i++) if(dow[i]>dow[busyIdx]) busyIdx=i;
  return { period, label, sub, inc, exp, net:inc-exp, txCount, days:days.size,
           topCat, topMerch, biggest, busyDay:['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][busyIdx],
           busyDayFull:['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][busyIdx],
           showIncome: S.trackIncome || inc>0 };
}
let _wrapData=null;
function openWrapped(data){
  if(!data) return;
  _wrapData=data;
  const el=document.getElementById('wrapped-overlay'); if(!el) return;
  const money=v=>fmtMoney(v);
  const stat=(lbl,val,accent)=>`<div class="wrap-stat"><div class="wrap-stat-l">${lbl}</div><div class="wrap-stat-v${accent?' accent':''}">${val}</div></div>`;
  let cards='';
  cards+=`<div class="wrap-hero-num">${esc(money(data.exp))}</div><div class="wrap-hero-sub">spent across <b>${data.txCount}</b> ${data.txCount===1?'entry':'entries'}</div>`;
  const grid=[];
  if(data.topCat) grid.push(stat('Top category', esc(data.topCat.name)+' · '+esc(money(data.topCat.amt)), true));
  if(data.topMerch) grid.push(stat('Most spent at', esc(data.topMerch.name), true));
  if(data.biggest) grid.push(stat('Biggest single', esc(money(data.biggest.amt))+' · '+esc(data.biggest.desc)));
  grid.push(stat('Busiest day', data.busyDayFull));
  grid.push(stat('Days tracked', String(data.days)));
  if(data.showIncome) grid.push(stat('Money in', esc(money(data.inc)), true));
  if(data.showIncome) grid.push(stat('Net', (data.net<0?'-':'+')+esc(money(Math.abs(data.net)))));
  document.getElementById('wrap-title').textContent='Prizm Wrapped';
  document.getElementById('wrap-period').textContent=data.label;
  document.getElementById('wrap-sub').textContent=data.sub;
  document.getElementById('wrap-hero').innerHTML=cards;
  document.getElementById('wrap-grid').innerHTML=grid.join('');
  el.classList.add('open');
  try{ if(navigator.vibrate) navigator.vibrate([12,40,12]); }catch(e){}
}
function closeWrapped(){ const el=document.getElementById('wrapped-overlay'); if(el) el.classList.remove('open'); }
function maybeShowWrapped(){
  try{
    if(document.getElementById('shell').style.display==='none') return;
    const now=new Date();
    const pm=new Date(now.getFullYear(), now.getMonth()-1, 1);
    const mKey='prizm_wrapped_m_'+pm.getFullYear()+'-'+(pm.getMonth()+1);
    if(now.getDate()<=12 && !localStorage.getItem(mKey)){
      const d=buildWrapped('month');
      if(d && d.txCount>=3){ localStorage.setItem(mKey,'1'); openWrapped(d); return; }
      if(d===null || (d&&d.txCount<3)) localStorage.setItem(mKey,'1'); // don't nag with a thin month
    }
    if(now.getMonth()>=6){ // July+ → the FY ending 30 Jun this year has closed
      const fKey='prizm_wrapped_fy_'+now.getFullYear();
      if(!localStorage.getItem(fKey)){
        const d=buildWrapped('fy');
        if(d && d.txCount>=5){ localStorage.setItem(fKey,'1'); openWrapped(d); return; }
      }
    }
  }catch(e){}
}
// Manual re-open from Settings (doesn't respect the once-only gate).
function openWrappedManual(){ const d=buildWrapped('month')||buildWrapped('fy'); if(d) openWrapped(d); else toast('Not enough activity yet for a wrap-up'); }

// ── Siri / Google Assistant quick-add via URL ─────────────────────────────────
// A Shortcut/Routine that opens  https://<app>/?quickadd=8.75%20coffee  logs it.
function handleQuickAddURL(){
  try{
    const p=new URLSearchParams(location.search);
    const raw=p.get('quickadd'); if(!raw) return;
    // clear the param so a refresh doesn't double-log
    try{ history.replaceState(null,'',location.pathname); }catch(e){}
    if(!S.accts||!S.accts.length){ toast('Add an account first, then try again'); return; }
    const parsed=_quickParseLine(raw);
    if(!parsed){ toast('Couldn\'t read that — try like "8.75 coffee"'); return; }
    const cat=parsed.category || (getCatList(parsed.type==='income').find(c=>c.n==='Other')||{n:'Other'}).n;
    apiPost({ action:'addTransaction', id:'tx_'+Date.now(), ts:Date.now(), type:parsed.type,
      amount:parsed.amount, category:cat, description:parsed.description,
      account:(S.accts[0]&&S.accts[0].name)||'', date:parsed.date, notes:'', memberId:(S.member&&S.member.id)||null });
    try{ celebrateAdd(); }catch(e){}
    toast('✓ Logged '+fmtMoney(parsed.amount)+' · '+parsed.description);
  }catch(e){}
}

// ── SCRIBE CONFIRM · a clean, colourful popup to verify a parsed entry ────────
let _sc = null;
function openScribeConfirm(p){
  const first = (S.accts && S.accts[0] && S.accts[0].name) || '';
  _sc = {
  ...
  const cur = document.getElementById('sc-cur'); if (cur) cur.textContent = (S.cur && S.cur.sym) || '$';
  const amt = document.getElementById('sc-amt'); if (amt) amt.textContent = _sc.amount ? String(_sc.amount) : '0';
  const desc = document.getElementById('sc-desc'); if (desc) desc.value = _sc.description;
  _scRenderCats(); _scRenderAccts(); _scRenderDates();
  openOverlay('scribe-confirm');
  try { if (navigator.vibrate) navigator.vibrate(8); } catch(e){}
}
function _scAmtEdit(){ const el=document.getElementById('sc-amt'); if(!el) return; const v=parseFloat((el.textContent||'').replace(/[^0-9.]/g,'')); _sc.amount = isNaN(v)?0:v; }
function _scDescEdit(){ const el=document.getElementById('sc-desc'); if(el) _sc.description = el.value; }
function _scRenderCats(){
  const wrap=document.getElementById('sc-cats'); if(!wrap) return;
  const list=getCatList(_sc.type==='income');
  wrap.innerHTML=list.slice(0,12).map(c=>{
    const on=c.n===_sc.category; const hue=getCatHue(c.n);
    return `<div class="sc-chip${on?' on':''}" style="${on?`background:hsl(${hue},48%,56%);border-color:transparent;color:#fff`:''}" onclick="_scPickCat('${esc(jsq(c.n))}')">${getCatIcon(c.n, _sc.type==='income')} ${esc(c.n)}</div>`;
  }).join('');
}
function _scPickCat(n){ _sc.category=n; _scRenderCats(); }
function _scRenderAccts(){
  const wrap=document.getElementById('sc-accts'); const block=document.getElementById('sc-acct-block');
  if(!wrap||!block) return;
  if(!S.accts||!S.accts.length){ block.style.display='none'; return; }
  block.style.display='';
  wrap.innerHTML=S.accts.map(a=>{
    const on=a.name===_sc.account;
    return `<div class="sc-chip${on?' on':''}" onclick="_scPickAcct('${esc(jsq(a.name))}')">${getAcctIcon(a.type)||'🏦'} ${esc(a.name)}</div>`;
  }).join('');
}
function _scPickAcct(n){ _sc.account=n; _scRenderAccts(); }
function _scRenderDates(){
  const wrap=document.getElementById('sc-dates'); if(!wrap) return;
  const today=todayISO(), yest=isoLocal(new Date(Date.now()-86400000));
  const opts=[['Today',today],['Yesterday',yest]];
  wrap.innerHTML=opts.map(o=>`<div class="sc-chip${_sc.date===o[1]?' on':''}" onclick="_scPickDate('${o[1]}')">${o[0]}</div>`).join('')
    + `<div class="sc-chip${(_sc.date!==today&&_sc.date!==yest)?' on':''}">${esc(fmtDateShort(_sc.date))}</div>`;
}
function _scPickDate(d){ _sc.date=d; _scRenderDates(); }
async function scribeConfirmSave(){
  if(!_sc || !(_sc.amount>0)){ toast('Enter an amount'); return; }
  if((!S.accts||!S.accts.length)){ toast('Add an account first'); closeOverlay('scribe-confirm'); openAcctModal(); return; }
  const desc=(_sc.description||'').trim() || (_sc.type==='income'?'Income':'Expense');
  const id='tx_'+Date.now()+'_'+Math.random().toString(36).slice(2,6);
  closeOverlay('scribe-confirm');
  await apiPost({ action:'addTransaction', id, date:_sc.date, description:desc, amount:_sc.amount, type:_sc.type, category:_sc.category, account:_sc.account, notes:'', memberId:(S.member&&S.member.id)||'', ts:Date.now() });
  try{ learnMerchant(desc, _sc.category, 2); }catch(e){}
  celebrateAdd();
  toast('✓ '+(_sc.type==='income'?'Income':'Saved')+' '+fmtMoney(_sc.amount));
  renderAll();
}
